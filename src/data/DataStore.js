...
function validateCue(cue, label) {
...
export function validateLoreConfig(lore, references) {
...
async function readJson(path) {
    ...
    if (!response.ok)
        throw new Error(`Không thể tải ${path}: ${response.status}`);
...
export async function loadGameData() {
    const [characters, weapons, passives, evolutions, enemies, stages, upgrades, metaUpgrades, lore] = await Promise.all([
        ...
        readJson('./data/passives.json'),
        readJson('./data/evolutions.json'),
        readJson('./data/enemies.json'),
        readJson('./data/stages.json'),
        readJson('./data/upgrades.json'),
        readJson('./data/meta-upgrades.json'),
    ...
    for (const weapon of weapons) {
        if (weapon.levels.length !== 8)
            throw new Error(`Vũ khí ${weapon.id} phải có đúng 8 cấp.`);

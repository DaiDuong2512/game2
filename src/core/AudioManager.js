 ...
 */
export const CLASS_SKILL_ACCENTS = {
...
export class SoundVoiceBudget {
    ...
    recentStarts = [];
    nextToken = 1;
    constructor(maxVoices = 12, maxStartsPerWindow = 6, burstWindowMs = 48) {
    ...
    reserve(id, nowMs) {
        ...
        if (lastSound !== undefined && nowMs - lastSound < profile.cooldownMs)
            return null;
        const lastGroup = this.lastByGroup.get(profile.group);
        if (profile.priority < 4 && lastGroup !== undefined && nowMs - lastGroup < profile.groupCooldownMs)
            return null;
        // Canh bao, No va Tuyet ky van phat trong mot frame day dan/hit.
        if (profile.priority < 4 && this.recentStarts.length >= this.maxStartsPerWindow)
            return null;
        let preemptToken;
        if (this.active.size >= this.maxVoices) {
            ...
            if (!weakest || profile.priority <= weakest.priority)
                return null;
    ...
    prune(nowMs) {
        for (const [token, voice] of this.active)
            if (voice.endsAtMs <= nowMs)
                this.active.delete(token);
        while ((this.recentStarts[0] ?? Number.POSITIVE_INFINITY) <= nowMs - this.burstWindowMs)
...
export class AudioManager {
    ...
    async ensureContext() {
        if (!this.unlocked && !navigator.userActivation?.hasBeenActive)
            return null;
        this.unlocked = true;
        if (!this.context) {
            try {
    ...
    playClassSkill(kind, intensity = 1) {
        this.play('class-skill', intensity * 0.82);
        const accent = CLASS_SKILL_ACCENTS[kind];
        if (accent)
            this.play(accent, intensity * 0.42);
    }
    playRagePhase(phase, intensity = 1) {
        this.play(RAGE_SOUND_CUES[phase], intensity);
    }
    playUltimatePhase(phase, intensity = 1) {
        this.play(ULTIMATE_SOUND_CUES[phase], intensity);
    }
    async playInternal(id, intensity) {
        ...
        if (this.settings.masterVolume <= 0 || this.settings.effectsVolume <= 0 || document.hidden)
            return;
        ...
        if (!reservation)
            return;
        if (reservation.preemptToken !== undefined)
            this.fadeOutVoice(reservation.preemptToken, context.currentTime);
        const profile = SOUND_PROFILES[id];
        ...
        const voice = { output, sources: [], remainingSources: 0 };
        ...
        for (const layer of profile.tones)
            this.addTone(context, now, output, voice, layer, pitchScale, reservation.token);
        if (profile.noise && this.noiseBuffer)
            this.addNoise(context, now, output, voice, profile.noise, reservation.token);
        if (voice.remainingSources === 0)
            this.finishVoice(reservation.token);
    }
    /** Chuỗi biến thiên xen kẽ ngăn hai phát liên tiếp nghe giống hệt nhau. */
    nextPitchScale(id, jitter) {
        if (jitter <= 0)
            return 1;
        const pattern = [-0.58, 0.34, -0.16, 0.66, -0.38, 0.11, 0.47, -0.05];
    ...
    addTone(context, now, output, voice, layer, pitchScale, token) {
        ...
        if (layer.endFrequency !== undefined)
            oscillator.frequency.exponentialRampToValueAtTime(Math.max(24, layer.endFrequency * pitchScale), end);
    ...
    addNoise(context, now, output, voice, layer, token) {
        if (!this.noiseBuffer)
            return;
    ...
    applyEnvelope(gain, start, end, attack, release, peak) {
    ...
    registerSource(voice, source, token) {
        ...
        source.addEventListener('ended', () => {
            ...
            if (current.remainingSources <= 0)
                this.finishVoice(token);
        }, { once: true });
    }
    fadeOutVoice(token, now) {
    ...
    updateAmbient(dt) {
        ...
        if (this.ambientTimer > 0 || this.settings.masterVolume <= 0.05 || this.settings.effectsVolume <= 0.05 || document.hidden)
            return;

...
export class UIManager {
    ...
    bindChoiceKeyboard(selector, expectedState) {
        this.unbindUpgradeKeyboard();
        const cards = [...this.screenRoot.querySelectorAll(selector)];
        if (cards.length === 0)
            return;
        let selectedIndex = 0;
        ...
        this.upgradeKeyHandler = (event) => {
            if (this.game.state !== expectedState)
                return;
            ...
            if (event.code === 'Enter') {
                if (event.target?.closest('#reroll-upgrades'))
                    return;
                event.preventDefault();
                event.stopPropagation();
        ...
        queueMicrotask(() => {
            if (this.game.state === expectedState && cards[0]?.isConnected)
                focusCard(0);
        });
    }
    unbindUpgradeKeyboard() {

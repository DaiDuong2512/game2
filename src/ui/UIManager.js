...
import { formatPlayerStatTransition, formatPlayerStatValue, PLAYER_STAT_GROUPS, PLAYER_STAT_LABELS, } from './StatPresentation.js';
export function chargedSkillStatus(meter, threshold, activeSeconds = 0) {
    if (activeSeconds > 0)
        return `Đang bật ${formatDecimal(activeSeconds, 1)} giây`;
    const value = Math.max(0, Math.floor(Number.isFinite(meter) ? meter : 0));
    return value >= threshold ? `Sẵn sàng · ${value}%` : `${value}/${threshold}%`;
}
export function chargedSkillFill(meter, threshold, activeSeconds = 0) {
    if (activeSeconds > 0)
        return 1;
...
const statLabels = PLAYER_STAT_LABELS;
...
export class UIManager {
    ...
    constructor(game, screenRoot, hudRoot, toastRoot) {
        this.game = game;
    ...
    showCharacterSelect() {
        ...
        const effectiveStats = new Map(this.game.data.characters.map((character) => {
            ...
            return [character.id, preview.stats];
        }));
      ...
      <section class="screen character-select-screen" style="--selection-backdrop:url('${asset(selectionBackdrop)}')">
        ...
        this.screenRoot.querySelectorAll('[data-character]').forEach((card) => {
            const choose = () => this.game.selectCharacter(card.dataset.character ?? '');
    ...
    showStageSelect() {
      ...
      <section class="screen stage-select-screen">
        ...
        this.screenRoot.querySelectorAll('[data-stage]').forEach((card) => {
            const choose = () => this.game.selectStage(card.dataset.stage ?? '');
            card.addEventListener('click', choose);
    ...
    showShop() {
      ...
      <section class="screen shop-screen">
        ...
        this.screenRoot.querySelectorAll('[data-meta]').forEach((button) => {
    ...
    showSettings() {
      ...
      <section class="screen settings-screen">
        ...
        this.bindBack(() => this.game.showMainMenu());
        this.screenRoot.querySelectorAll('input[type="range"][data-setting]').forEach((input) => {
        ...
        this.screenRoot.querySelectorAll('[data-toggle]').forEach((button) => {
            button.addEventListener('click', () => {
                ...
                if (state)
                    state.textContent = value ? 'Bật' : 'Tắt';
            });
        });
        this.screenRoot.querySelectorAll('[data-color-blind]').forEach((select) => {
            select.addEventListener('change', () => {
                this.game.updateSettings({ colorBlindMode: select.value });
                this.applyAccessibilitySettings();
            });
        });
        this.byId('reset-save')?.addEventListener('click', () => {
            if (window.confirm('Xóa toàn bộ tiến trình, cài đặt và nội dung đã mở khóa? Thao tác này không thể hoàn tác.'))
                this.game.resetSave();
        });
    }
    showGameplayHUD() {
    ...
    updateHUD(force = false) {
        if (this.hudRoot.classList.contains('hidden') || !this.game.runStats || !this.game.player)
            return;
        const now = performance.now();
        if (!force && now - this.lastHudUpdate < 70)
            return;
        ...
        const playerWithClassCooldown = player;
        ...
        if (blessing) {
            blessing.classList.toggle('hidden', !player.bossBlessingActive);
            if (player.bossBlessingActive) {
    ...
    updateWeaponStrip() {
        ...
        for (const entry of withSlot) {
            ...
            const mask = strip.querySelector(`[data-cooldown="${entry.config.id}"]`);
            if (!level || !mask)
                continue;
    ...
    showPause() {
      ...
      <div class="overlay">
        ...
        this.byId('restart-run')?.addEventListener('click', () => {
            if (window.confirm('Chơi lại bản đồ và hủy toàn bộ tiến trình của trận hiện tại?'))
                this.game.startSelectedRun();
        });
        this.byId('abandon-run')?.addEventListener('click', () => {
            if (window.confirm('Kết thúc trận và cất số vàng đã thu thập?'))
    ...
    showLevelUp(options) {
      ...
      <div class="overlay upgrade-overlay">
        <section class="overlay-card" role="dialog" aria-modal="true" aria-labelledby="level-up-title">
          ...
          <p>Chọn 1 trong ${options.length}. Ưu tiên hiệu ứng chữ ký và cộng hưởng với bộ vũ khí hiện tại.</p>
          ...
          <div class="upgrade-actions">
            ...
            <button type="button" class="btn" id="skip-upgrade" ${this.game.upgrades.skips <= 0 ? 'disabled' : ''}>Bỏ qua · ${this.game.upgrades.skips}</button>
          </div>
        </section>
      </div>`;
        this.screenRoot.querySelectorAll('[data-upgrade]').forEach((button) => {
        ...
        this.byId('skip-upgrade')?.addEventListener('click', () => this.game.skipUpgrade());
    }
    showStartingLoadout(options) {
        this.setMobileControlsActive(false);
        const cards = options.map((option, index) => {
      ...
      <div class="overlay upgrade-overlay starter-overlay">
        ...
        this.screenRoot.querySelectorAll('[data-starter]').forEach((button) => {
            button.addEventListener('click', () => this.game.chooseStarterOption(button.dataset.starter ?? ''));
        });
    }
    showPermanentRewards(choices) {
    ...
    showSummary(stats, unlocks, permanentRewards = []) {
        this.hideHUD();
    ...
    showMissionBriefing(briefing, onStart, onCodex) {
        this.hideHUD();
    ...
    setHUDObjective(objective) {
        this.hudObjectiveOverride = objective.trim();
        if (this.hudObjectiveOverride)
            this.setText('hud-objective', this.hudObjectiveOverride);
    }
    /** Điểm nối cho lời thoại ngắn trong trận; không dừng hoặc che chiến trường. */
    showTransmission(cue, portrait, duration = 4200) {
        if (this.transmissionTimer !== undefined)
    ...
    header(kicker, title, actionId, actionLabel, action = false) {
    ...
    rangeSetting(label, description, key, value, min, max, step) {
    ...
    toggleSetting(label, description, key, value) {
    ...
    colorBlindSetting(value) {
        const options = Object.entries(colorBlindLabels)
    ...
    characterAbility(kind, name, description) {
    ...
    skillButton(id, name, key, kindLabel, stateId, meterId, variant = '', icon = 'assets/generated/weapons/arcane-nova-v2.png') {
    ...
    abilityIcon(kind) {
        ...
        if (kind === 'ultimate')
            return ultimateIcons[characterId] ?? 'assets/generated/weapons/arcane-nova-v2.png';
    ...
    weaponCardFacts(weaponId, requestedLevel = 1) {
    ...
    bindPermanentRewards() {
        this.screenRoot.querySelectorAll('[data-permanent]').forEach((button) => {
            button.addEventListener('click', () => this.game.claimPermanentReward(button.dataset.permanent ?? ''));
        });
    }
    permanentStatLabel(stat) {
        const labels = {
    ...
    permanentEffectLabel(stat, points) {
        const effect = permanentPointEffect(stat, points);
        if (stat === 'armor')
            return `+${formatDecimal(effect.value, 1)} Giáp`;
        return `+${formatDecimal(effect.value * 100, 2)}%`;
    }
    permanentProgressionSummary() {
        const points = this.game.saveSystem.data.permanentPoints;
        const stats = Object.keys(points).filter((stat) => points[stat] > 0);
    ...
    damageSourceTitle(id) {
        if (id.startsWith('active-'))
            return 'Kỹ năng chủ động';
        if (id.startsWith('rage-'))
            return 'Nộ nhân vật';
        if (id.startsWith('ultimate-'))
            return 'Tuyệt kỹ nhân vật';
        if (id.includes('burn') || id.includes('poison') || id.includes('status'))
            return 'Hiệu ứng theo thời gian';
        if (id.includes('self-destruct'))
            return 'Tự hủy của kẻ địch';

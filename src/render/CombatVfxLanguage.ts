export type CombatCueTier = 'primary' | 'active' | 'rage' | 'ultimate';
export type ImpactWeight = 'hit' | 'skill' | 'finisher';
 ...
 */
export function combatCueTier(source: string): CombatCueTier {
  const normalized = source.trim().toLowerCase();
  if (normalized === 'ultimate' || normalized.startsWith('ultimate-')) return 'ultimate';
  if (normalized === 'rage' || normalized.startsWith('rage-')) return 'rage';
  if (normalized === 'active' || normalized.startsWith('active-')) return 'active';
  return 'primary';
}

export function combatCueProfile(source: string): CombatCueProfile {
  const normalized = source.trim().toLowerCase();
...
export function impactWeightForSize(size: number): ImpactWeight {
  if (size >= 180) return 'finisher';
  if (size >= 76) return 'skill';
  return 'hit';
}

...
export class Renderer {
    ...
    render(scene) {
        ...
        if (!gpuBackground)
            this.drawBackground(scene.stage);
    ...
    drawTerrainGround(stage) {
        ...
        ctx.globalAlpha = 0.5;
        ctx.drawImage(image, -overflowX * 0.5 + parallaxX, -overflowY * 0.5 + parallaxY, drawWidth, drawHeight);
        ctx.globalAlpha = 1;
        ...
        if (propsAtlas) {
            for (const feature of terrain.features()) {
                if (feature.kind === 'water')
                    this.drawTerrainFeature(propsAtlas, feature);
            }
        }
        if (grassAtlas) {
            ...
            for (const patch of terrain.decorations()) {
                if (!this.camera.isVisible(patch.x, patch.y, 52))
                    continue;
                ...
                ctx.drawImage(grassAtlas, column * cellWidth, row * cellHeight, cellWidth, cellHeight, -size * 0.5, -size * 0.5, size, size);
                ctx.restore();
            }
        }
        if (propsAtlas) {
    ...
    drawTerrainFeature(atlas, feature) {
        const margin = feature.kind === 'water' ? 110 : 90;
        if (!this.camera.isVisible(feature.x, feature.y, margin))
            return;
        ...
        this.context.drawImage(atlas, column * cellWidth, row * cellHeight, cellWidth, cellHeight, Math.round(screen.x - width * 0.5), Math.round(screen.y - height * (feature.kind === 'water' ? 0.5 : 0.68)), width, height);
    ...
    drawEnemies(scene) {
        ...
        for (const enemy of scene.enemies.pool.allItems()) {
            ...
            if (screen.x < -margin || screen.x > this.width + margin || screen.y < -margin || screen.y > this.height + margin)
                continue;
            if (renderedEnemies >= aftermathLimit)
                continue;
            renderedEnemies += 1;
    ...
    drawContainedSprite(image, size) {
    ...
    drawSpawnPortals(scene) {
        const image = this.assets.get(VOID_DEVOURER_ABILITY_PATH);
        const ctx = this.context;
        for (const enemy of scene.enemies.pool.allItems()) {
            if (!enemy.active || enemy.spawnPortalTime <= 0 || !this.camera.isVisible(enemy.x, enemy.y, enemy.radius * 4))
                continue;
            const screen = this.camera.worldToScreen(enemy.x, enemy.y);
            ...
            if (image)
                ctx.drawImage(image, Math.round(screen.x - size / 2), Math.round(screen.y - size / 2), Math.round(size), Math.round(size));
            else {
    ...
    drawProjectiles(scene) {
        ...
        scene.projectiles.pool.forEachActive((projectile) => {
            if (projectile.persistent || !this.camera.isVisible(projectile.x, projectile.y, 40))
                return;
            if (renderedProjectiles >= aftermathLimit)
                return;
    ...
    drawFloatingText(scene) {
        ...
        scene.floatingText.pool.forEachActive((text) => {
            if (!this.camera.isVisible(text.x, text.y, 50))
                return;
            if (renderedText >= aftermathLimit)
                return;
            renderedText += 1;

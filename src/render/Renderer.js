...
import { SWORD_MAX_VISIBLE_LAYERS, swordLayerRadius } from '../game/WeaponSystem.js';
import { combatCueProfile, combatCueTier, impactWeightForSize } from './CombatVfxLanguage.js';
import { GpuCanvasPresenter } from './GpuCanvasPresenter.js';
...
const FACING_TO_SPRITE_ROW = [6, 7, 0, 1, 2, 3, 4, 5];
...
export function bossIndicatorPoint(originX, originY, targetX, targetY, width, height, inset = 58) {
...
export class Renderer {
    ...
    render(scene) {
        ...
        const gpuBackground = this.presenter.beginGpuFrame(scene.stage.theme.background, scene.stage.theme.accent, leftWorld, topWorld);
        if (!gpuBackground)
            this.drawBackground(scene.stage, scene.time);
        ...
        if (scene.settings.damageNumbers)
            this.drawFloatingText(scene);
        this.drawBossDirectionIndicator(scene);
        this.drawScreenEdge(scene.player);
        this.presenter.present();
    }
    drawBackground(stage, _time) {
        const ctx = this.context;
        ctx.globalAlpha = 1;
        ...
        const leftWorld = this.camera.x - this.width * 0.5 - this.camera.shakeX;
        const topWorld = this.camera.y - this.height * 0.5 - this.camera.shakeY;
        const firstColumn = Math.floor(leftWorld / spacing);
        const lastColumn = Math.ceil((leftWorld + this.width) / spacing);
        for (let column = firstColumn; column <= lastColumn; column += 1) {
            const x = Math.round(column * spacing - leftWorld);
            const major = column % 4 === 0;
            ctx.fillStyle = major ? 'rgba(159, 184, 187, .13)' : 'rgba(159, 184, 187, .065)';
            ctx.fillRect(x, 0, major ? 2 : 1, this.height);
        }
        const firstRow = Math.floor(topWorld / spacing);
        const lastRow = Math.ceil((topWorld + this.height) / spacing);
        for (let row = firstRow; row <= lastRow; row += 1) {
            const y = Math.round(row * spacing - topWorld);
            const major = row % 4 === 0;
            ctx.fillStyle = major ? 'rgba(159, 184, 187, .13)' : 'rgba(159, 184, 187, .065)';
            ctx.fillRect(0, y, this.width, major ? 2 : 1);
        }
        const landmarkSpacing = spacing * 8;
        const firstLandmarkX = Math.floor(leftWorld / landmarkSpacing);
        const lastLandmarkX = Math.ceil((leftWorld + this.width) / landmarkSpacing);
        const firstLandmarkY = Math.floor(topWorld / landmarkSpacing);
        const lastLandmarkY = Math.ceil((topWorld + this.height) / landmarkSpacing);
        ctx.fillStyle = hexToRgba(stage.theme.accent, 0.18);
        for (let gx = firstLandmarkX; gx <= lastLandmarkX; gx += 1) {
            for (let gy = firstLandmarkY; gy <= lastLandmarkY; gy += 1) {
                const x = Math.round(gx * landmarkSpacing - leftWorld);
                const y = Math.round(gy * landmarkSpacing - topWorld);
                ctx.fillRect(x - 5, y - 1, 11, 3);
                ctx.fillRect(x - 1, y - 5, 3, 11);
            }
        }
    }
    drawTelegraphs(scene) {
        const ctx = this.context;
        ...
        scene.boss.telegraphs.forEachActive((telegraph) => {
            ...
            if (!this.camera.isVisible(telegraph.x, telegraph.y, telegraph.radius + 20))
                return;
            ...
            if (telegraph.kind === 'circle')
                this.drawHatchedCircle(screen.x, screen.y, telegraph.radius, compact ? 22 : 13, compact ? 1 : 2);
            else
                this.drawHatchedAnnulus(screen.x, screen.y, telegraph.radius * 0.72, compact ? 52 : 44, compact ? 31 : 24);
            ...
            if (!this.presenter.addGroundRing(telegraph.x, telegraph.y, telegraph.radius, 3 + Math.round(progress * 3), compact ? 30 : 48, ringColor, ringAlpha)) {
            ...
            if (!this.presenter.addGroundRing(telegraph.x, telegraph.y, closingRadius, progress > 0.72 ? 4 : 2, compact ? 18 : 28, ringColor, ringAlpha)) {
            ...
            for (let index = 0; index < orbitMarks; index += 1) {
                ...
                ctx.fillRect(Math.round(screen.x + Math.cos(angle) * radius) - 2, Math.round(screen.y + Math.sin(angle) * radius) - 2, 4, 4);
            }
            if (progress > 0.72) {
        ...
        if (!castCue || !this.camera.isVisible(castCue.x, castCue.y, castCue.radius + 30))
            return;
        ...
        ctx.fillStyle = color;
        if (!this.presenter.addGroundRing(castCue.x, castCue.y, castCue.radius * pulse, 3 + Math.round(castCue.progress * 3), 28, color, castAlpha)) {
        ...
        for (let index = 0; index < castRays; index += 1) {
            ...
            this.drawSteppedLine(screen.x + Math.cos(angle) * outer, screen.y + Math.sin(angle) * outer, screen.x + Math.cos(angle) * inner, screen.y + Math.sin(angle) * inner, 3 + castCue.phase, 6);
        }
    ...
    drawBossAbilityVisuals(scene) {
        for (const visual of scene.boss.getAbilityVisuals()) {
            if (!this.camera.isVisible(visual.x, visual.y, visual.radius + 40))
                continue;
            ...
            this.drawBossAbilityFrame(visual.bossId, 1, screen.x, screen.y, visual.radius * 2.35 * pulse, Math.sin(Math.min(1, progress) * Math.PI) * 0.92);
        }
    }
    drawUltimateField(scene) {
        ...
        switch (kind) {
            ...
            case 'plague-night': {
                ...
                for (let index = 0; index < count; index += 1) {
                    ...
                    ctx.fillRect(Math.round(x + Math.cos(angle) * radius - size / 2), Math.round(y + Math.sin(angle) * radius - size / 2 - Math.sin(scene.time * 3 + index) * 7), size, size);
                }
                ctx.globalAlpha = 0.2;
    ...
    drawPersistentZones(scene) {
        const ctx = this.context;
        scene.projectiles.pool.forEachActive((projectile) => {
            if (!projectile.persistent || !this.camera.isVisible(projectile.x, projectile.y, projectile.radius + 30))
                return;
            ...
            if (row !== undefined) {
                ...
                this.drawVfxFrame(row, frame, screen.x, screen.y, Math.min(210, Math.max(42, radius * 1.5)), projectile.element === 'arcane' ? 0.11 : 0.24);
            }
            ...
            if (projectile.element === 'poison') {
                ...
                for (let index = 0; index < 10; index += 1) {
                    ...
                    ctx.fillRect(Math.round(screen.x + Math.cos(angle) * distance) - 2, Math.round(screen.y + Math.sin(angle) * distance) - 2, 4, 4);
                }
            }
            ctx.restore();
        });
    }
    drawToxicSmokeZone(scene, projectile, centerX, centerY, radius) {
        const ctx = this.context;
    ...
    drawEnemies(scene) {
        ...
        for (const enemy of scene.enemies.pool.allItems()) {
            ...
            if (screen.x < -margin || screen.x > this.width + margin || screen.y < -margin || screen.y > this.height + margin)
                continue;
            const image = this.assets.get(enemy.config.sprite);
            const bossAtlas = enemy.isBoss ? this.assets.get(BOSS_CHARACTER_ATLAS_PATH) : null;
            ...
            if (detailed)
                this.drawEnemyStatusAura(enemy, scene.time, size);
            if (detailed && enemy.status.poisonCloudTime > 0 && enemy.status.slowTime > 0) {
                ...
                if (!insideToxicZone)
                    this.drawPoisonResidualCue(enemy, scene.time);
            }
            ctx.save();
            ...
            ctx.scale(1 + attackMotion * 0.14 + chargeStretch * 0.08 + hitHold * 0.05 - squashWave, 1 - attackMotion * 0.09 - hitHold * 0.04 + squashWave);
            ctx.globalAlpha = enemy.alpha;
            if (bossAtlas && BOSS_ATLAS_INDEX[enemy.config.id] !== undefined) {
            ...
            if (detailed && rangedRecoil > 0.05) {
                ...
                this.drawVfxFrame(muzzleRow, Math.min(5, Math.floor((1 - rangedRecoil) * 6)), towardPlayerX * enemy.radius * 1.2, towardPlayerY * enemy.radius * 1.2, enemy.radius * 2.2, rangedRecoil * 0.82);
            }
            ...
            if (detailed)
                this.drawEnemyStatusIcons(enemy, x, Math.round(y + size * 0.52 + 7), scene.time);
        }
    }
    drawBossCharacterFrame(image, bossId, size) {
        ...
        this.context.drawImage(image, column * sourceWidth, 0, sourceWidth, sourceHeight, Math.round(-width / 2), Math.round(-height / 2), Math.round(width), Math.round(height));
    }
    drawBossAbilityFrame(bossId, row, x, y, size, alpha) {
        ...
        ctx.drawImage(image, column * sourceWidth, Math.max(0, Math.min(BOSS_ABILITY_ROWS - 1, row)) * sourceHeight, sourceWidth, sourceHeight, Math.round(x - size / 2), Math.round(y - size / 2), Math.round(size), Math.round(size));
        ctx.restore();
    }
    drawProjectiles(scene) {
        const ctx = this.context;
        scene.projectiles.pool.forEachActive((projectile) => {
            if (projectile.persistent || !this.camera.isVisible(projectile.x, projectile.y, 40))
                return;
            const screen = this.camera.worldToScreen(projectile.x, projectile.y);
            const x = Math.round(screen.x);
            const y = Math.round(screen.y);
            ...
            if (hostile) {
                ...
                this.drawSteppedLine(x - directionX * projectile.radius * 2.8, y - directionY * projectile.radius * 2.8, x, y, Math.max(5, projectile.radius * 0.88), 3);
                ctx.fillStyle = hostileColor;
                this.drawSteppedLine(x - directionX * projectile.radius * 2.7, y - directionY * projectile.radius * 2.7, x - directionX * projectile.radius * 0.35, y - directionY * projectile.radius * 0.35, Math.max(2, projectile.radius * 0.4), 3);
                ...
                this.drawSteppedLine(x - directionX * (arm + 8) + sideX * 6, y - directionY * (arm + 8) + sideY * 6, x - directionX * arm, y - directionY * arm, 2, 3);
                this.drawSteppedLine(x - directionX * (arm + 8) - sideX * 6, y - directionY * (arm + 8) - sideY * 6, x - directionX * arm, y - directionY * arm, 2, 3);
                if (!dangerous) {
                ...
                ctx.fillStyle = '#031014';
                this.drawSteppedLine(x - directionX * projectile.radius * 1.9, y - directionY * projectile.radius * 1.9, x + directionX * projectile.radius * 2.1, y + directionY * projectile.radius * 2.1, Math.max(5, projectile.radius * 0.86), 3);
                ctx.fillStyle = '#c9fbff';
                this.drawSteppedLine(x - directionX * projectile.radius * 1.45, y - directionY * projectile.radius * 1.45, x + directionX * projectile.radius * 1.75, y + directionY * projectile.radius * 1.75, Math.max(3, projectile.radius * 0.56), 3);
                ctx.fillStyle = color;
                if (aegisSniperRay) {
                    const sideX = -directionY;
                    const sideY = directionX;
                    ctx.fillStyle = '#10252b';
                    this.drawSteppedLine(x - directionX * 24, y - directionY * 24, x + directionX * 10, y + directionY * 10, Math.max(5, projectile.radius * 0.92), 3);
                    ctx.fillStyle = '#9ef6ff';
                    this.drawSteppedLine(x - directionX * 20, y - directionY * 20, x + directionX * 13, y + directionY * 13, Math.max(2, projectile.radius * 0.42), 3);
                    ...
                    this.drawSteppedLine(x - directionX * projectile.radius * 1.8, y - directionY * projectile.radius * 1.8, x + directionX * projectile.radius * 2.5, y + directionY * projectile.radius * 2.5, Math.max(3, projectile.radius * 0.55), 3);
                    ...
                    this.drawSteppedLine(x - directionX * projectile.radius * 1.4 + sideX * 5, y - directionY * projectile.radius * 1.4 + sideY * 5, x - directionX * projectile.radius * 0.55, y - directionY * projectile.radius * 0.55, 2, 3);
                    this.drawSteppedLine(x - directionX * projectile.radius * 1.4 - sideX * 5, y - directionY * projectile.radius * 1.4 - sideY * 5, x - directionX * projectile.radius * 0.55, y - directionY * projectile.radius * 0.55, 2, 3);
                }
                    ...
                    for (let index = 1; index <= 3; index += 1) {
                        ...
                        ctx.fillRect(Math.round(x - directionX * distance + sideX * (index % 2 === 0 ? 2 : -2)) - 1, Math.round(y - directionY * distance + sideY * (index % 2 === 0 ? 2 : -2)) - 1, size, size);
                    }
                }
                else if (projectile.element === 'poison') {
                    ...
                    this.drawSteppedLine(x - directionX * projectile.radius, y - directionY * projectile.radius, x + directionX * projectile.radius * 1.7, y + directionY * projectile.radius * 1.7, Math.max(2, projectile.radius * 0.42), 2);
                }
                ...
                if (projectile.element === 'fire') {
                    for (let index = 1; index <= 3; index += 1) {
                        ...
                        ctx.fillRect(Math.round(x - directionX * (projectile.radius * (1.4 + index * 0.65)) - 1 + -directionY * flicker), Math.round(y - directionY * (projectile.radius * (1.4 + index * 0.65)) - 1 + directionX * flicker), index === 1 ? 3 : 2, index === 1 ? 3 : 2);
                    }
                }
                ...
                if (combatTier === 'active') {
                    ...
                    this.drawSteppedLine(x - directionX * tail + sideX * 7, y - directionY * tail + sideY * 7, x - directionX * projectile.radius * 0.5, y - directionY * projectile.radius * 0.5, 2, 3);
                    this.drawSteppedLine(x - directionX * tail - sideX * 7, y - directionY * tail - sideY * 7, x - directionX * projectile.radius * 0.5, y - directionY * projectile.radius * 0.5, 2, 3);
                }
                else if (combatTier === 'ultimate') {
                    const pulse = 1 + Math.sin(scene.time * 18 + projectile.id) * 0.08;
                    ctx.fillStyle = '#ffe69a';
                    this.drawCornerBrackets(x, y, Math.max(9, projectile.radius * 1.7 * pulse), Math.max(4, projectile.radius * 0.72), 2);
                }
     ...
     */
    drawAbilityCastCue(scene) {
        const player = scene.player;
        if (player.animationState !== 'cast' || player.actionKind !== 'ability')
            return;
        ...
        if (profile.tier === 'active') {
            ...
            for (let index = 0; index < 4; index += 1) {
                ...
                this.drawSteppedLine(screen.x + Math.cos(angle) * radius, screen.y + Math.sin(angle) * radius, screen.x + Math.cos(angle) * arm, screen.y + Math.sin(angle) * arm, 3, 3);
            }
    ...
    drawWeaponCompanions(scene) {
        ...
        for (const entry of scene.weapons.entries()) {
            ...
            if (!level)
                continue;
            if (entry.config.behavior !== 'orbit' && entry.config.behavior !== 'summon' && entry.config.behavior !== 'slash')
                continue;
            ...
            for (let index = 0; index < count; index += 1) {
                ...
                for (let blade = 0; blade < bladesPerLayer; blade += 1) {
                    ...
                    ctx.save();
                    ctx.translate(x, y);
                    if (entry.config.behavior === 'slash')
    ...
    drawBossDirectionIndicator(scene) {
        const boss = scene.boss.getBoss();
        if (!boss || this.camera.isVisible(boss.x, boss.y, boss.radius + 24))
            return;
        const playerScreen = this.camera.worldToScreen(scene.player.x, scene.player.y);
    ...
    drawPlayer(player, time, reducedEffects) {
        const ctx = this.context;
        const screen = this.camera.worldToScreen(player.x, player.y);
        const x = Math.round(screen.x);
        const y = Math.round(screen.y);
        ...
        this.drawSteppedLine(x + player.aim.x * aimGuideStart, y + player.aim.y * aimGuideStart, x + player.aim.x * aimGuideEnd, y + player.aim.y * aimGuideEnd, 4, 3);
        ctx.fillStyle = '#ffe58a';
        this.drawSteppedLine(x + player.aim.x * (aimGuideStart + 2), y + player.aim.y * (aimGuideStart + 2), x + player.aim.x * (aimGuideEnd - 3), y + player.aim.y * (aimGuideEnd - 3), 2, 2);
        ctx.restore();
        if (player.ultimateActive > 0) {
        ...
        if (player.lightSoldierTime > 0)
            this.drawLightSoldier(player, x, y, time);
        ...
        if (spriteSheet) {
            ...
            const bob = player.animationState === 'run' ? Math.round(anticipation - plantStrength * 2)
                : player.animationState === 'dash' ? -2
                    : player.animationState === 'hurt' ? Math.round(1 + Math.sin(time * 42))
                        : actionActive ? Math.round(actionAnticipation
            ...
            const leanX = Math.round(directionX * (player.animationState === 'dash' ? 4 : 2.2 * player.movementBlend - anticipation * 1.6 + recovery)
                + actionDirectionX * actionLean
                + directionX * hurtRecoil * 3);
            const leanY = Math.round(directionY * (player.animationState === 'dash' ? 2 : player.movementBlend - anticipation + recovery * 0.55)
            ...
            if (player.animationState === 'dash') {
                ...
                for (let layer = 2; layer >= 1; layer -= 1) {
                    ...
                    ctx.translate(Math.round(-directionX * (12 + layer * 11)), Math.round(feetY - directionY * (7 + layer * 7)));
                    ctx.scale(1.08 + smearStrength * 0.06, 0.92 - smearStrength * 0.05);
                    ctx.drawImage(spriteSheet, frame * sourceCellWidth, row * sourceCellHeight, sourceCellWidth, sourceCellHeight, Math.round(-size / 2), Math.round(-size * footY / sourceCellHeight), size, size);
                    ctx.restore();
                }
            }
            ctx.save();
    ...
    drawLightSoldier(player, playerX, playerY, time) {
    ...
    drawGuardianPassiveFrame(index, x, y, size, alpha = 1) {
        ...
        ctx.save();
        ctx.globalAlpha *= clamp(alpha, 0, 1);
        ctx.drawImage(image, safeIndex % columns * sourceWidth, Math.floor(safeIndex / columns) * sourceHeight, sourceWidth, sourceHeight, Math.round(x - size / 2), Math.round(y - size / 2), Math.round(size), Math.round(size));
        ctx.restore();
        return true;
    }
    drawHeldPrimaryWeapon(player, feetY, visualScale, time, reducedEffects, leanX = 0) {
        const ctx = this.context;
        ...
        const handY = Math.round(shoulderY
        ...
        if (casting) {
            ...
            this.drawPlayerCastGesture(player.abilityCastKind, handX + aimX * 7 * scale, handY + aimY * 5 * scale, release, recovery, scale, reducedEffects);
            ctx.restore();
            return;
        }
        if (attacking && behavior === 'slash' && release > 0.18 && !reducedEffects) {
            ...
            this.drawSteppedLine(handX - perpendicularX * 8 * scale, handY - perpendicularY * 8 * scale, handX + aimX * 18 * scale, handY + aimY * 18 * scale, pixel + 1, 3);
            ctx.globalAlpha = 0.98;
        }
        switch (behavior) {
            case 'slash':
    ...
    drawPlayerCastGesture(kind, centerX, centerY, release, recovery, scale, reducedEffects) {
        const ctx = this.context;
    ...
    drawParticles(scene) {
        ...
        scene.particles.pool.forEachActive((particle) => {
            if (!this.camera.isVisible(particle.x, particle.y, particle.size + 60))
                return;
    ...
    drawAtlasVfx(scene) {
        scene.particles.atlasPool.forEachActive((effect) => {
            if (!this.camera.isVisible(effect.x, effect.y, effect.size * 0.6 + 20))
                return;
            ...
            if (effect.sheet === 'status')
                this.drawStatusVfxFrame(effect.row, frame, screen.x, screen.y, renderSize * scale, alpha);
            else
                this.drawVfxFrame(effect.row, frame, screen.x, screen.y, renderSize * scale, alpha);
    ...
    drawImpactWeightEnvelope(weight, centerX, centerY, size, progress, alpha, reduced) {
        if (weight === 'hit')
            return;
        const ctx = this.context;
        ...
        if (weight === 'skill') {
            ...
            for (let index = 0; index < marks; index += 1) {
                ...
                this.drawSteppedLine(centerX + Math.cos(angle) * radius, centerY + Math.sin(angle) * radius, centerX + Math.cos(angle) * inner, centerY + Math.sin(angle) * inner, 2, 3);
            }
        }
        ctx.restore();
    }
    drawImpactSemantic(semantic, centerX, centerY, size, progress, alpha) {
        const ctx = this.context;
    ...
    drawFloatingText(scene) {
        ...
        scene.floatingText.pool.forEachActive((text) => {
            if (!this.camera.isVisible(text.x, text.y, 50))
                return;
    ...
    drawDamageTextGlyph(kind, centerX, centerY, radius, color) {
        const ctx = this.context;
    ...
    drawProjectileAtlasSprite(projectile, atlasIndex, x, y, directionX, directionY) {
        ...
        ctx.rotate(angle);
        ctx.drawImage(image, sourceX, sourceY, cellWidth, cellHeight, Math.round(-drawSize / 2), Math.round(-drawSize / 2), drawSize, drawSize);
        ctx.restore();
        return true;
    }
    drawVfxFrame(row, frame, x, y, size, alpha = 1) {
        ...
        ctx.drawImage(image, safeFrame * VFX_CELL_WIDTH, safeRow * VFX_CELL_HEIGHT, VFX_CELL_WIDTH, VFX_CELL_HEIGHT, Math.round(-drawSize / 2), Math.round(-drawSize / 2), drawSize, drawSize);
        ctx.restore();
        return true;
    }
    drawStatusVfxFrame(row, frame, x, y, size, alpha = 1) {
        ...
        ctx.globalAlpha *= clamp(alpha, 0, 1);
        ctx.translate(Math.round(x), Math.round(y));
        ctx.drawImage(image, safeFrame * STATUS_VFX_CELL_SIZE + STATUS_VFX_INSET, safeRow * STATUS_VFX_CELL_SIZE + STATUS_VFX_INSET, sourceSize, sourceSize, Math.round(-drawSize / 2), Math.round(-drawSize / 2), drawSize, drawSize);
        ctx.restore();
        return true;
    }
    drawToxicSmokeFrame(row, frame, x, y, size, alpha = 1) {
        ...
        ctx.globalAlpha *= clamp(alpha, 0, 1);
        ctx.translate(Math.round(x), Math.round(y));
        ctx.drawImage(image, safeFrame * TOXIC_SMOKE_CELL_SIZE, safeRow * TOXIC_SMOKE_CELL_SIZE, TOXIC_SMOKE_CELL_SIZE, TOXIC_SMOKE_CELL_SIZE, Math.round(-drawSize / 2), Math.round(-drawSize / 2), drawSize, drawSize);
        ctx.restore();
        return true;
    }
    drawPixelRing(centerX, centerY, radius, blockSize, segments = 32) {
        const ctx = this.context;
        ...
        for (let index = 0; index < count; index += 1) {
            ...
            ctx.fillRect(Math.round(centerX + Math.cos(angle) * safeRadius - size * 0.5), Math.round(centerY + Math.sin(angle) * safeRadius - size * 0.5), size, size);
        }
    }
    drawHatchedCircle(centerX, centerY, radius, spacing, thickness) {
        const ctx = this.context;
        ...
        for (let offsetY = -safeRadius; offsetY <= safeRadius; offsetY += step) {
            ...
            ctx.fillRect(Math.round(centerX - width * 0.5), Math.round(centerY + offsetY), Math.round(width), line);
        }
    }
    drawHatchedAnnulus(centerX, centerY, radius, thickness, segments) {
        ...
        for (let index = 0; index < count; index += 1) {
            if (index % 2 !== 0)
                continue;
            const angle = index / count * TAU;
            this.drawSteppedLine(centerX + Math.cos(angle) * (radius - halfThickness), centerY + Math.sin(angle) * (radius - halfThickness), centerX + Math.cos(angle) * (radius + halfThickness), centerY + Math.sin(angle) * (radius + halfThickness), 3, 5);
        }
    }
    drawRadialCracks(centerX, centerY, radius, count, offset, width) {
        for (let index = 0; index < count; index += 1) {
            ...
            this.drawSteppedLine(midX, midY, centerX + Math.cos(angle) * radius, centerY + Math.sin(angle) * radius, Math.max(2, width - 2), 5);
        }
    }
    drawHazardDiamond(centerX, centerY, radius, color) {
        const ctx = this.context;
    ...
    drawCornerBrackets(centerX, centerY, arm, corner, thickness) {
        const ctx = this.context;
    ...
    drawPlayerProjectileStar(centerX, centerY, radius) {
        const ctx = this.context;
    ...
    drawSteppedLine(startX, startY, endX, endY, pixelSize, stepSize) {
        const ctx = this.context;
        ...
        for (let index = 0; index <= count; index += 1) {
            ...
            ctx.fillRect(Math.round(startX + deltaX * progress - size * 0.5), Math.round(startY + deltaY * progress - size * 0.5), size, size);
        }
    }
    drawPixelDiamond(radius, color, centerColor) {
        const ctx = this.context;
    ...
    drawEnemyStatusAura(enemy, time, size) {
        let row = null;
        if (enemy.status.blindTime > 0)
            row = 3;
        else if (enemy.status.paralysisTime > 0 || enemy.status.shockTime > 0)
            row = 0;
        else if (enemy.status.burnTime > 0)
            row = 1;
        else if (enemy.status.slowTime > 0)
            row = 2;
        else if (enemy.status.stunTime > 0)
            row = 3;
        else if (enemy.shield > 0)
            row = 4;
        const frame = Math.floor(time * 10 + enemy.id) % 6;
        if (row !== null)
            this.drawVfxFrame(row, frame, 0, 0, Math.round(size * 1.18), row === 3 ? 0.2 : 0.34);
        const ctx = this.context;
        ctx.save();
    ...
    drawPoisonResidualCue(enemy, time) {
        const ctx = this.context;
    ...
    drawEnemyStatusIcons(enemy, centerX, y, time) {
        if (enemy.status.burnTime <= 0
        ...
        const statuses = [];
        if (enemy.status.bleedTime > 0)
            statuses.push({ row: -4, color: '#c8443f' });
        if (enemy.status.burnTime > 0)
            statuses.push({ row: 1, color: '#ff7546' });
        if (enemy.status.slowTime > 0)
            statuses.push({ row: 2, color: '#72d8ff' });
        if (enemy.status.shockTime > 0 || enemy.status.paralysisTime > 0)
            statuses.push({ row: 0, color: '#62a7ff' });
        if (enemy.status.blindTime > 0)
            statuses.push({ row: 3, color: '#c879ff' });
        if (enemy.status.poisonTime > 0 || enemy.status.poisonCloudTime > 0)
            statuses.push({ row: -1, color: '#68dc72' });
        if (enemy.status.stunTime > 0)
            statuses.push({ row: -2, color: '#ffe76a' });
        if (enemy.status.healingReduction > 0)
            statuses.push({ row: -3, color: '#ff7869' });
        if (enemy.shield > 0)
            statuses.push({ row: 4, color: '#72d8ff' });

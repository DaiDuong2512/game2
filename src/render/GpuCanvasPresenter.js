...
void main() {
  ...
  vec2 gridCell = floor(world / 56.0);
  vec2 gridEdge = min(fract(world / 56.0), 1.0 - fract(world / 56.0)) * 56.0;
  float minorGrid = float(gridEdge.x < 1.0 || gridEdge.y < 1.0);
  float majorGrid = float(
    (gridEdge.x < 2.0 && mod(gridCell.x, 4.0) == 0.0) ||
    (gridEdge.y < 2.0 && mod(gridCell.y, 4.0) == 0.0)
  );
  background = mix(background, vec3(0.624, 0.722, 0.733), minorGrid * 0.065 + majorGrid * 0.065);

  vec2 landmark = abs(mod(world + 224.0, 448.0) - 224.0);
  float landmarkMark = float(
    (landmark.x < 5.5 && landmark.y < 1.5) ||
    (landmark.x < 1.5 && landmark.y < 5.5)
  );
  background = mix(background, uAccent, landmarkMark * 0.18);

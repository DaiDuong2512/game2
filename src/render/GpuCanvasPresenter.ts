export type RenderBackend = 'webgl2-compositor' | 'canvas2d';

export interface GpuRenderStats {
  backend: RenderBackend;
  groundRings: number;
  estimatedCanvasCommandsAvoided: number;
  displayPixelRatio: number;
  dynamicRenderScale: number;
}
...
interface GpuPipeline {
  ...
  themeUniform: WebGLUniformLocation;
  accentUniform: WebGLUniformLocation;
  viewportUniform: WebGLUniformLocation;
  worldOriginUniform: WebGLUniformLocation;
  ringCountUniform: WebGLUniformLocation;
  ringGeometryUniform: WebGLUniformLocation;
  ringColorUniform: WebGLUniformLocation;
  ringSegmentsUniform: WebGLUniformLocation;
}
...
export class GpuCanvasPresenter {
  ...
  private gpu: GpuPipeline | null;
  private cssWidth = 1;
  private cssHeight = 1;
  private displayPixelRatio = 1;
  private readonly themeColor = new Float32Array([0.063, 0.094, 0.11]);
  private readonly accentColor = new Float32Array([0.18, 0.7, 0.65]);
  private worldOriginX = 0;
  private worldOriginY = 0;
  private ringCount = 0;
  private estimatedCanvasCommandsAvoided = 0;
  private readonly ringGeometry = new Float32Array(MAX_GROUND_RINGS * 4);
  private readonly ringColor = new Float32Array(MAX_GROUND_RINGS * 4);
  private readonly ringSegments = new Float32Array(MAX_GROUND_RINGS);
  ...
  public constructor(canvas: HTMLCanvasElement) {
    ...
    if (gl) {
      ...
      if (!context) throw new Error('Không thể khởi tạo lớp dựng gameplay hai chiều.');
      ...
      if (!context) throw new Error('Không thể khởi tạo vùng vẽ hai chiều.');
  ...
  public resize(width: number, height: number, displayPixelRatio: number): void {
  ...
  public beginGpuFrame(theme: string, accent: string, worldOriginX: number, worldOriginY: number): boolean {
    if (!this.gpu || this.gpu.gl.isContextLost()) return false;
  ...
  public addGroundRing(
    ...
    if (!this.gpu || this.gpu.gl.isContextLost() || this.ringCount >= MAX_GROUND_RINGS) return false;
  ...
  public present(): void {
    const gpu = this.gpu;
    if (!gpu || gpu.gl.isContextLost()) return;
    const { gl } = gpu;
  ...
  private createGpuPipeline(gl: WebGL2RenderingContext): GpuPipeline {
    ...
    if (!program) throw new Error('Không thể tạo chương trình WebGL2.');
    ...
    if (!vertexArray) throw new Error('Không thể tạo tài nguyên WebGL2.');
  ...
  private configureGpuTarget(): void {
    const gpu = this.gpu;
    if (!gpu || gpu.gl.isContextLost()) return;
  ...
  private requireUniform(gl: WebGL2RenderingContext, program: WebGLProgram, name: string): WebGLUniformLocation {
    ...
    if (!uniform) throw new Error(`Không tìm thấy uniform WebGL2 ${name}.`);
    return uniform;
  }

  private readHexColor(hex: string, target: Float32Array, offset = 0): void {
  ...
  private compileShader(gl: WebGL2RenderingContext, type: number, source: string): WebGLShader {
    ...
    if (!shader) throw new Error('Không thể tạo shader WebGL2.');

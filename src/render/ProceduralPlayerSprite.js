...
export function getProceduralPlayerStyle(characterId) {
    return PROCEDURAL_PLAYER_STYLES[characterId] ?? DEFAULT_STYLE;
}
/** Tạo pose thuần để renderer và test dùng chung, không phụ thuộc Canvas. */
export function createProceduralPlayerPose(input) {
    ...
    return {
        ...
        leanX: Math.round(facing.x * (input.animationState === 'dash' ? 3 : 1.5 * blend - anticipation * 1.2 + recovery * 0.7)
            + actionDirectionX * actionLean + recoilX * 2.6 + hurtJolt),
        leanY: Math.round(facing.y * (input.animationState === 'dash' ? 2 : blend - anticipation * 0.8 + recovery * 0.45)
...
function weaponVisualPalette(behavior) {
    switch (behavior) {
...
function block(painter, x, y, width, height, color) {
    painter.ctx.fillStyle = painter.flash > 0.55 ? painter.style.highlight : color;
    painter.ctx.fillRect(Math.round(x * painter.unit), Math.round(y * painter.unit), Math.max(painter.unit, Math.round(width * painter.unit)), Math.max(painter.unit, Math.round(height * painter.unit)));
}
function pixelLine(painter, x, y, x2, y2, color, width = 1) {
    const dx = x2 - x;
    const dy = y2 - y;
...
function torsoHalfWidth(style) {
    if (style.silhouette === 'colossus')
        return 6;
    if (style.silhouette === 'guardian')
        return 5;
    if (style.silhouette === 'shepherd')
        return 4.5;
    return 4;
}
/**
 ...
 */
function drawPortraitBackDetails(painter) {
...
function drawCapeAndRearDetails(painter) {
    const { style, pose } = painter;
    ...
    if (pose.directionY < -0.35)
        block(painter, -3 + pose.headOffsetX, -25 + pose.headOffsetY, 6, 4, style.hair);
}
function drawLegs(painter) {
    const { style, pose } = painter;
...
function drawArms(painter) {
    ...
    const handX = Math.round(gripAimX * (aimLength + throwDistance - recoilDistance) + perpendicularX * gripSideOffset);
    const handY = -14 + Math.round(gripAimY * (3 + Math.max(0, throwDistance))
...
function drawPrimaryWeapon(painter, handX, handY, aimX, aimY, offhandSide, offHandX, offHandY) {
    const { style, pose } = painter;
    const behavior = painter.primaryWeaponBehavior;
...
function drawCastGesture(painter, focusX, focusY) {
    const { pose } = painter;
...
function drawPortraitFrontDetails(painter) {
    const { style, pose } = painter;
    const headX = pose.headOffsetX;
    const headY = -27 + pose.headOffsetY;
    const facingSide = pose.sideFacing === 0 ? 1 : pose.sideFacing;
...
export function drawProceduralPlayerSprite(ctx, input) {
    ...
    if (input.animationState === 'dash') {
        ...
        for (let layer = 2; layer >= 1; layer -= 1) {
            ...
            ctx.translate(Math.round(-pose.directionX * (5 + layer * 5) * unit), Math.round(-pose.directionY * (3 + layer * 3) * unit));
...
//# sourceMappingURL=ProceduralPlayerSprite.js.map
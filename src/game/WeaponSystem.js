import { angleDelta, distance, distanceSquared, normalize, pointToSegmentDistanceSquared, TAU, } from '../core/MathUtils.js';
...
const WEAPON_BALANCE_DAMAGE_MULTIPLIERS = {
    slash: 1.18,
    bow: 1,
    gun: 1.1,
    darts: 1.16,
...
export function weaponBalanceDamageMultiplier(behavior) {
...
export function characterWeaponCountBonus(characterId, weaponId) {
    return characterId === 'zarek' && weaponId === 'venom-bloom' ? 1 : 0;
}
export function swordLayerRadius(baseRange, layerIndex) {
    return baseRange + Math.max(0, layerIndex) * SWORD_LAYER_SPACING;
}
/**
 * Số tia lẻ giữ đội hình quạt có tia giữa. Số tia chẵn ghép từng tia tiến
 * với một tia đối diện 180°, nên tia đầu luôn khóa đúng tâm mục tiêu.
 */
export function volleyAngleOffset(index, count, spread) {
    if (count <= 1)
        return 0;
    if (count % 2 === 1)
        return (index - Math.floor(count / 2)) * spread;
    const spoke = Math.floor(index / 2);
    const spokeOffset = spoke === 0 ? 0 : Math.ceil(spoke / 2) * spread * (spoke % 2 === 1 ? 1 : -1);
    return spokeOffset + (index % 2 === 1 ? Math.PI : 0);
}
const DIRECTIONAL_BEHAVIORS = new Set([
    'bow',
    'gun',
    'darts',
    'bomb',
...
export function weaponFireSound(weapon) {
...
function playDirectSignatureHit(world, effect) {
    const cue = signatureHitSound(effect);
    if (cue)
        world.audio?.play(cue, cue === 'bleed' ? 0.24 : 0.2);
}
class DataDrivenWeapon extends WeaponBase {
    data;
    foldedRateMultiplier = 1;
    foldedVolleyMultiplier = 1;
    swordPulsesRemaining = 0;
    swordPulseInterval = 0.25;
    constructor(config, data, slot) {
        super(config, slot);
        this.data = data;
    }
    update(dt, world, assignedTarget) {
    ...
    fire(world, evolution, assignedTarget) {
        const level = this.levelData();
        const requestedCount = Math.max(1, level.count
            + (evolution?.countBonus ?? 0)
            + world.player.stats.get('bonusProjectiles')
        ...
        if (this.runtime.slot === 'primary')
            world.player.triggerPrimaryAttack?.(this.config.behavior, baseAngle);
        // Chỉ một nhánh được phát cho mỗi lần khai hỏa, nên cue nhận diện không
        // chồng thêm tiếng `shoot` chung phía sau.
        if (this.config.behavior === 'slash')
            world.audio?.play('slash', 0.3);
        else if (this.config.behavior === 'bow')
            world.audio?.play('bow', 0.26);
        else if (this.config.behavior === 'lightning')
            world.audio?.play('lightning', 0.3);
        else if (this.config.behavior === 'fireball')
            world.audio?.play('fire', 0.26);
        else if (this.config.behavior === 'poison-bomb')
            world.audio?.play('poison-throw', 0.48);
        else if (this.config.behavior === 'poison')
            world.audio?.play('poison-cloud', 0.22);
        else if (this.config.element === 'arcane')
            world.audio?.play('arcane', 0.22);
        else
            world.audio?.play('shoot', this.config.behavior === 'bomb' ? 0.24 : 0.16);
        switch (this.config.behavior) {
            case 'slash':
                this.fireSwordPulse(world, evolution);
                break;
            case 'bow':
                this.fireProjectiles(world, count, baseAngle, speed, range, color, 0.13, level.pierce, level.size, 0, false, evolution);
                break;
            case 'gun':
                this.fireProjectiles(world, count, baseAngle, speed, range, color, 0.045, level.pierce, level.size, 0, false, evolution);
                break;
            case 'darts':
                this.fireProjectiles(world, count, baseAngle, speed, range, color, 0.17, level.pierce, level.size, 0, false, evolution);
                break;
            case 'bomb':
                this.fireBomb(world, target, aim, speed, range, color, count, evolution);
                break;
            ...
            case 'lightning':
                this.fireLightning(world, target ?? world.nearestEnemy(world.player.x, world.player.y, range), range, color, count, evolution);
                break;
            case 'fireball':
                this.fireFireball(world, count, baseAngle, speed, range, color, evolution);
                break;
            case 'ice':
                this.fireIce(world, count, speed, range, color, evolution);
                break;
            case 'laser':
                this.fireLaser(world, range, baseAngle, color, count, evolution);
                break;
            case 'poison':
                this.firePoison(world, target, aim, range, color, count, evolution);
                break;
            case 'poison-bomb':
                this.firePoisonBomb(world, range, speed, color, count, evolution);
                break;
            case 'orbit':
                this.fireOrbit(world, range, color, count, evolution);
                break;
            case 'summon':
                this.fireSummon(world, range, speed, color, count, evolution);
                break;
            case 'nova':
                this.fireNova(world, count, speed, range, color, evolution);
                break;
        }
    }
    rollDamage(world, evolution, canCrit = true) {
        const level = this.levelData();
        ...
        if (critical)
            damage *= world.player.effectiveCritDamage();
        return { damage, critical };
    }
    fireSwordPulse(world, evolution, pulseIndex = 0) {
        const level = this.levelData();
        const requestedCount = Math.max(1, level.count + (evolution?.countBonus ?? 0) + world.player.stats.get('bonusProjectiles'));
        ...
        for (const enemy of candidates) {
            ...
            for (let layer = 0; layer < visibleBlades; layer += 1) {
                if (enemyDistance <= swordLayerRadius(baseRange, layer) + enemy.radius)
                    layersInReach += 1;
            }
            if (layersInReach === 0)
                continue;
            const hit = this.rollDamage(world, evolution);
            world.damageEnemy(enemy, hit.damage * SWORD_PULSE_DAMAGE_SHARE * layersInReach, this.config.element, this.config.id, this.statusChance(level.statusChance), level.knockback * SWORD_PULSE_KNOCKBACK_SHARE, hit.critical, world.player.x, world.player.y, this.config.signature);
        ...
        for (let layer = 0; layer < visibleBlades; layer += 1) {
            ...
            world.particles.line(x - tangentX * halfBlade, y - tangentY * halfBlade, x + tangentX * halfBlade, y + tangentY * halfBlade, elementColors[this.config.element], 6, this.swordPulseInterval * 0.9);
    ...
    fireProjectiles(world, count, baseAngle, speed, range, color, spread, pierce, size, homing, explosive, evolution) {
        for (let index = 0; index < count; index += 1) {
            const angle = baseAngle + volleyAngleOffset(index, count, spread);
            const hit = this.rollDamage(world, evolution);
    ...
    fireBomb(world, target, aim, speed, range, color, count, evolution) {
        const level = this.levelData();
        for (let index = 0; index < count; index += 1) {
    ...
    fireLightning(world, target, range, color, count, evolution) {
        if (!target)
            return;
    ...
    fireFireball(world, count, baseAngle, speed, range, color, evolution) {
        const level = this.levelData();
        for (let index = 0; index < count; index += 1) {
    ...
    fireIce(world, count, speed, range, color, evolution) {
        const level = this.levelData();
    ...
    fireLaser(world, range, baseAngle, color, count, evolution) {
        const level = this.levelData();
        ...
        for (let beam = 0; beam < beams; beam += 1) {
            ...
            for (const enemy of candidates) {
                if (!enemy.active)
                    continue;
                if (pointToSegmentDistanceSquared(enemy.x, enemy.y, world.player.x, world.player.y, endX, endY) > (enemy.radius + level.size * 0.48) ** 2)
                    continue;
                const hit = this.rollDamage(world, evolution);
    ...
    firePoison(world, target, aim, range, color, count, evolution) {
        const level = this.levelData();
        for (let index = 0; index < count; index += 1) {
    ...
    firePoisonBomb(world, range, speed, color, count, evolution) {
        const level = this.levelData();
    ...
    densestClusterTarget(world, range, clusterRadius) {
        ...
        for (const candidate of candidates) {
            ...
            for (const neighbor of candidates) {
                if (distanceSquared(candidate.x, candidate.y, neighbor.x, neighbor.y) <= clusterRadius * clusterRadius)
                    nearbyCount += 1;
            }
    ...
    fireOrbit(world, range, color, count, evolution) {
        const level = this.levelData();
        ...
        for (let index = 0; index < count; index += 1) {
            ...
            for (const enemy of nearby) {
                if (!enemy.active || distanceSquared(x, y, enemy.x, enemy.y) > (level.size + enemy.radius) ** 2)
                    continue;
                const hit = this.rollDamage(world, evolution);
    ...
    fireSummon(world, range, speed, color, count, evolution) {
        const level = this.levelData();
    ...
    fireNova(world, count, speed, range, color, evolution) {
        const level = this.levelData();
...
export class WeaponSystem {
    static MAX_AUXILIARY_WEAPONS = 3;
    data;
    weapons = [];
    constructor(data) {
        this.data = data;
    }
    addWeapon(id, requestedSlot) {
        if (this.weapons.some((weapon) => weapon.config.id === id))
            return false;
        const config = this.data.weaponById.get(id);
        if (!config)
            return false;
        const slot = requestedSlot ?? (this.primaryEntry() ? 'auxiliary' : 'primary');
        if (slot === 'primary' && this.primaryEntry())
            return false;
        if (slot === 'auxiliary' && !this.canAddAuxiliary())
            return false;
        this.weapons.push(new DataDrivenWeapon(config, this.data, slot));
        return true;
    }
    equipPrimaryWeapon(id) {
        return this.addWeapon(id, 'primary');
    }
    addAuxiliaryWeapon(id) {
    ...
    masterWeapon(id) {
        ...
        if (!weapon || weapon.runtime.level < weapon.config.maxLevel)
            return false;
        weapon.addMastery();
        return true;
    }
    refineWeapon(id, bonus) {
        const weapon = this.weapons.find((item) => item.config.id === id);
        if (!weapon || !Number.isFinite(bonus) || bonus <= 0)
            return false;
        weapon.runtime.refinementBonus += bonus;
        return true;
    }
    evolve(id, evolutionId) {
        const weapon = this.weapons.find((item) => item.config.id === id);
        const evolution = this.data.evolutionById.get(evolutionId);
        if (!weapon || !evolution || weapon.runtime.evolutionId)
            return false;
    ...
    update(dt, world) {
        ...
        for (const weapon of this.weapons)
            weapon.update(dt, world, assignments.get(weapon));
    }
    planDirectionalTargets(world) {
        ...
        const plans = directional.map((weapon, slotIndex) => {
            ...
            for (const enemy of world.enemySpatial.queryCircle(world.player.x, world.player.y, range)) {
                if (!enemy.active || distanceSquared(world.player.x, world.player.y, enemy.x, enemy.y) > rangeSquared)
                    continue;
        ...
        for (const plan of plans) {
            const sorted = [...plan.candidates].sort((left, right) => {
                ...
                if (Math.abs(angularDifference) > 1e-9)

...
class DataDrivenWeapon extends WeaponBase {
    ...
    fire(world, evolution, assignedTarget) {
        ...
        switch (this.config.behavior) {
            case 'slash':
                ...
                break;
            case 'bow':
                this.fireProjectiles(world, count, baseAngle, speed, range, color, 0.13, level.pierce, level.size, 0, false, evolution, target);
                break;
            case 'gun':
                this.fireProjectiles(world, count, baseAngle, speed, range, color, 0.045, level.pierce, level.size, 0, false, evolution, target);
                break;
            case 'darts':
                this.fireProjectiles(world, count, baseAngle, speed, range, color, 0.17, level.pierce, level.size, 0, false, evolution, target);
                break;
            case 'bomb':
    ...
    fireProjectiles(world, count, baseAngle, speed, range, color, spread, pierce, size, homing, explosive, evolution, preferredTarget) {
        const nearbyTargets = [...new Map((world.autoAim ? world.enemySpatial.queryCircle(world.player.x, world.player.y, range) : [])
                .filter((enemy) => enemy.active)
                .map((enemy) => [enemy.id, enemy])).values()].sort((left, right) => distanceSquared(world.player.x, world.player.y, left.x, left.y)

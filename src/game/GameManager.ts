...
import type { MissionBriefing, NarrativeCue } from '../narrative/NarrativeTypes.js';
...
export function resolveQaModes(searchText: string): { qaMode: boolean; fastQaPacing: boolean } {
...
export class GameManager {
  ...
  public readonly meta: MetaProgression;
  public readonly narrative: NarrativeSystem;
  public readonly camera: Camera;
  public readonly renderer: Renderer;
  public readonly ui: UIManager;
  public readonly qaMode: boolean;
  public readonly fastQaPacing: boolean;
  ...
  public experience!: ExperienceSystem;
  public upgrades!: UpgradeSystem;
  public boss!: BossSystem;
  public passives!: CharacterPassiveSystem;
  public stageManager!: StageManager;
  public terrain!: TerrainSystem;
  public enemySpatial = new SpatialHash<Enemy>(128);
  public runStats: RunStats | null = null;
  public bossAftermathSpawned = 0;
  ...
  private bossAftermathElapsed = 0;
  private pendingLevelChoice = false;
  private bonusChoices = 0;
  private runCommitted = false;
  private summaryUnlocks: string[] = [];
  private worldOriginX = 0;
  private worldOriginY = 0;
  private lastPassiveRender = 0;
  private lastRenderedState: GameState | null = null;
  private activeBriefing: MissionBriefing | null = null;
  private pendingStoryEnding: NarrativeCue[] = [];
  private pendingCodexUnlocks: string[] = [];
  private hitStopRemaining = 0;
  private hitStopCooldown = 0;
  private dashTrailTimer = 0;
  private observedDashSerial = 0;
  private observedFootstepSerial = 0;
  ...
  private loop(timestamp: number): void {
    ...
    if (this.state === 'playing') this.input.pollGamepad();
    ...
    if (activeRunScene && shouldRender) {
      this.renderer.render({
        stage: this.stageManager.stage as StageConfig,
        time: this.totalTime,
        player: this.player,
  ...
  private updateGame(dt: number): void {
    if (this.input.wasPressed('Tab')) this.ui.toggleCharacterStats();
    ...
    if (this.victoryDelay > 0) {
      ...
      this.camera.update(
        ...
        this.player.x,
        this.player.y,
        this.saveSystem.data.settings.screenShake,
      );
      if (this.victoryDelay <= 0) this.finishRun('victory');
      return;
    }
    ...
    this.projectiles.setPlayerEmpowerment(
      this.player.bossAftermathProjectileSpeedMultiplier(),
      this.player.bossAftermathProjectileSizeMultiplier(),
    );
    ...
    for (const cue of this.narrative.updateProgress(storyProgress)) this.showNarrativeCue(cue);
    this.handleStageAnnouncements(stageEvent);

    const scaling = this.stageManager.scaling();
    if (aftermathWasActive) this.updateBossAftermathSpawns(dt);
    else {
      this.director.update(
        ...
        this.player.y,
        scaling,
        this.stageManager.wave,
        this.stageManager.intermission,
        this.spawner.pool.countActive(),
        this.renderer.size(),
        Boolean(this.boss.getBoss()),
      );
    }
    ...
    this.camera.update(
      ...
      this.player.y,
      this.saveSystem.data.settings.screenShake,
      this.player.motionVx,
      this.player.motionVy,
      this.player.aim.x,
      this.player.aim.y,
    );
  ...
  private spawnFinalEncounter(): void {
    ...
    if (finaleCue) this.showNarrativeCue(finaleCue, 5200);
    ...
    if (!stage.bossId) this.toast(`${enemy.config.name} — Tinh Anh xuất hiện`);
    this.camera.addShake(7);
  }

  private handleStageAnnouncements(stageEvent: { waveChanged: boolean; shouldSpawnFinal: boolean }): void {
    if (stageEvent.shouldSpawnFinal) {
      this.spawnFinalEncounter();
      return;
    }
    if (!stageEvent.waveChanged) return;
    this.audio.play('boss', 0.25);
    this.syncSurvivalObjective();
    this.toast(`Đợt ${this.stageManager.wave} — mật độ kẻ địch tăng`);
  }

  private syncSurvivalObjective(): void {
    const stage = this.stageManager.stage;
    if (!stage) return;
  ...
  public startSelectedRun(): void {
    ...
    if (!character || !stage) throw new Error('Không tìm thấy nhân vật hoặc bản đồ có thể chơi.');
    if (!this.isCharacterUnlocked(character.id) || !this.isStageUnlocked(stage.index)) return;

    const briefing = this.narrative.startStage(stage.id, character.id);
    this.activeBriefing = briefing;
    const showBriefing = (): void => {
      this.ui.showMissionBriefing(
        briefing,
        () => this.initializeSelectedRun(),
        () => this.showCodex(showBriefing),
      );
    };
    showBriefing();
  }

  private initializeSelectedRun(): void {
    const character = this.data.characterById.get(this.selectedCharacterId) ?? this.data.characters[0];
    const stage = this.data.stageById.get(this.selectedStageId) ?? this.data.stages[0];
    if (!character || !stage) throw new Error('Không tìm thấy nhân vật hoặc bản đồ có thể chơi.');
    if (!this.isCharacterUnlocked(character.id) || !this.isStageUnlocked(stage.index)) return;
    ...
    this.enemySpatial = new SpatialHash<Enemy>(128);
    this.victoryDelay = 0;
  ...
  public chooseStarterOption(optionId: string): void {
    if (this.state !== 'starting-loadout' || !this.upgrades.applyStarter(optionId)) return;
    this.ui.hideOverlay();
    this.beginCombat();
  }

  private beginCombat(): void {
    const stage = this.stageManager.stage as StageConfig;
    ...
    if (this.activeBriefing?.characterLine) this.showNarrativeCue(this.activeBriefing.characterLine, 3600);
  }
  ...
  public damageEnemy(
    ...
    if (!enemy.active || rawDamage <= 0) return { amount: 0, critical, killed: false };
    ...
    enemy.lastHitWeapon = sourceWeaponId;
    if (totalApplied > 0 && hitEffect) this.applyWeaponSignature(enemy, adjustedDamage, sourceWeaponId, hitEffect);
    ...
    if (this.rng.chance(statusChance)) {
      switch (element) {
        ...
        case 'ice':
          ...
          if (enemy.status.slowFactor < 0.2) enemy.status.stunTime = Math.max(enemy.status.stunTime, 0.35);
          break;
        case 'lightning':
        ...
        case 'arcane':
          ...
          } else if (this.rng.chance(0.18)) enemy.status.stunTime = Math.max(enemy.status.stunTime, 0.1);
          break;
      }
      if (this.player.character.passive.kind === 'status-echo' && this.rng.chance(this.player.character.passive.value)) {
        const excluded = new Set<number>([enemy.id]);
        ...
        if (echo) {
          switch (element) {
            ...
            case 'poison': echo.status.poisonTime = Math.max(echo.status.poisonTime, 2.6); echo.status.poisonDps = Math.max(echo.status.poisonDps, adjustedDamage * 0.065); break;
            case 'ice': echo.status.slowTime = Math.max(echo.status.slowTime, 1.4); echo.status.slowFactor = Math.min(echo.status.slowFactor, 0.68); break;
            case 'lightning': {
            ...
            case 'physical': echo.knockbackX += (echo.x - enemy.x) * 0.08; echo.knockbackY += (echo.y - enemy.y) * 0.08; break;
            case 'arcane': echo.status.stunTime = Math.max(echo.status.stunTime, 0.08); break;
          }
    ...
    if (this.weapons) this.weapons.recordDamage(sourceWeaponId, totalApplied);
    if (this.passives) this.passives.onDamageDealt(this, enemy, sourceWeaponId, totalApplied);
    ...
    if (critical) this.audio.play('crit', 0.22);
    ...
    if (killed) this.killEnemy(enemy, sourceWeaponId);
  ...
  public damageStatus(
    ...
    if (!enemy.active || rawDamage <= 0) return false;
    ...
    if (this.weapons) this.weapons.recordDamage(sourceWeaponId, applied);
  ...
  private applyWeaponSignature(
    enemy: Enemy,
    scaledDamage: number,
    sourceWeaponId: string,
    effect: WeaponSignatureConfig,
  ): void {
    if (!this.rng.chance(effect.chance ?? 1)) return;
  ...
  public killEnemy(enemy: Enemy, sourceWeaponId: string): void {
    ...
    if (configId === 'rift-splitter') {
      const scaling = this.stageManager.scaling();
      for (let index = 0; index < 2; index += 1) this.spawner.spawnChild('riftling', x, y, scaling, 0.55);
    }
    ...
    if (this.passives) this.passives.onEnemyKilled(this);
    this.spawner.pool.release(enemy);
    if (wasBoss) this.boss.clearBoss();
    if (wasFinal) {
      this.stageManager.bossDefeated = true;
      if (wasBoss) this.startBossAftermath();
      else {
        this.victoryDelay = this.qaMode ? 1.8 : 4.8;
        this.player.health = Math.max(1, this.player.health);
        this.player.invulnerable = Math.max(this.player.invulnerable, this.victoryDelay + 0.5);
  ...
  private startBossAftermath(): void {
    this.bossAftermathElapsed = 0;
    this.bossAftermathSpawned = 0;
  ...
  private updateBossAftermathSpawns(dt: number): void {
    ...
    const candidates = stage.allowedEnemies
      ...
      .filter((config): config is EnemyConfig => Boolean(
        config && config.tier === 'normal' && resolveEnemySizeClass(config) === 'small',
      ));
  ...
  public damagePlayer(rawDamage: number, sourceX: number, sourceY: number): void {
  ...
  public gainShards(amount: number): void {
    if (!this.runStats) return;
    this.runStats.shards += amount;
    this.toast(`+${amount} Mảnh Khe Nứt`);
  }

  public applyStatShard(statId: string): void {
    const previousMaxHp = this.player.stats.get('maxHp');
    const displayStat = (statId === 'heal' ? null : statId) as keyof PlayerStatBlock | null;
    ...
    switch (statId) {
      case 'damage': this.player.stats.apply('damage', 0.012, 'multiply'); break;
      case 'attackSpeed': this.player.stats.apply('attackSpeed', 0.01, 'multiply'); break;
      case 'moveSpeed': this.player.stats.apply('moveSpeed', 0.008, 'multiply'); break;
      case 'armor': this.player.stats.apply('armor', 0.35, 'add'); break;
      case 'maxHp': this.player.stats.apply('maxHp', 3, 'add'); break;
      case 'lifeSteal': this.player.stats.apply('lifeSteal', 0.001, 'add'); break;
      case 'hpRegen': this.player.stats.apply('hpRegen', 0.08, 'add'); break;
      case 'luck': this.player.stats.apply('luck', 0.006, 'add'); break;
      case 'bonusProjectiles': this.player.stats.apply('bonusProjectiles', 1, 'add'); break;
      case 'armorPenetration': this.player.stats.apply('armorPenetration', 0.006, 'add'); break;
      case 'statusResistance': this.player.stats.apply('statusResistance', 0.008, 'add'); break;
      case 'bodyScale':
        ...
        break;
      case 'critChance': this.player.stats.apply('critChance', 0.004, 'add'); break;
      case 'critDamage': this.player.stats.apply('critDamage', 0.025, 'add'); break;
      case 'heal': this.player.heal(this.player.stats.get('maxHp') * 0.08); break;
      default: this.player.stats.apply('damage', 0.01, 'multiply'); break;
    }
    this.player.syncMaxHp(previousMaxHp);
    this.player.statShards += 1;
    if (this.runStats) this.runStats.statShards += 1;
  ...
  public gainSkillCritShard(): void {
    this.player.skillCritShards += 1;
    if (this.runStats) this.runStats.skillCritShards += 1;
  ...
  private openNextUpgradeChoice(): void {
    ...
    if (!fromLevel) this.bonusChoices -= 1;
  ...
  public chooseUpgrade(optionId: string): void {
    if (this.state !== 'level-up') return;
    if (!this.upgrades.apply(optionId)) return;
  ...
  public banishUpgrade(optionId: string): void {
  ...
  private resumeAfterUpgrade(): void {
    this.ui.hideOverlay();
    this.state = 'playing';
    if (this.pendingLevelChoice) queueMicrotask(() => {
      if (this.state === 'playing') this.openNextUpgradeChoice();
    });
  }
  ...
  private finishRun(result: RunStats['result']): void {
    if (!this.runStats || this.runCommitted) return;
    this.runCommitted = true;
    this.runStats.result = result;
    this.runStats.elapsed = this.stageManager.elapsed;
    this.runStats.wave = this.stageManager.wave;
    this.runStats.level = this.player.level;
    for (const entry of this.weapons.entries()) this.runStats.damageByWeapon[entry.config.id] = entry.runtime.damageDealt;
    const stage = this.stageManager.stage as StageConfig;
    ...
    this.state = 'summary';
    const showSummary = (): void => this.ui.showSummary(this.runStats as RunStats, unlocks, permanentRewards);
    const ending = result === 'victory' ? this.pendingStoryEnding.splice(0) : [];
    if (ending.length > 0) this.ui.showStoryEnding(ending, showSummary);
    else showSummary();
    if (result !== 'victory') this.audio.play('defeat');
  }

  public claimPermanentReward(choiceId: string): void {
    ...
    if (!this.meta.claimPermanentReward(choiceId)) return;
    ...
    if (this.runStats) this.ui.showSummary(this.runStats, this.summaryUnlocks, []);
  ...
  public showCodex(onClose: () => void = () => this.showMainMenu()): void {
  ...
  public selectCharacter(id: string): void {
    if (!this.isCharacterUnlocked(id)) return;
  ...
  public selectStage(id: string): void {
    const stage = this.data.stageById.get(id);
    if (!stage || !this.isStageUnlocked(stage.index)) return;
  ...
  public startNextStage(): void {
    ...
    if (next && this.isStageUnlocked(next.index)) this.selectedStageId = next.id;
    this.showStageSelect();
  }

  public screenShake(amount: number): void {
  ...
  private showNarrativeCue(cue: NarrativeCue, duration = 4200): void {
  ...
  private completeNarrativeStage(): void {
    const completion = this.narrative.completeStage();
    const victoryCue = completion.cues.find((cue) => cue.kind === 'victory');
    if (victoryCue) this.showNarrativeCue(victoryCue, this.qaMode ? 1800 : 4600);
    ...
    if (completion.ending.length > 0) this.pendingStoryEnding = [...completion.ending];
  }

  public get autoAim(): boolean {
  ...
  private updateDashTrail(dt: number, previousX: number, previousY: number, dashing: boolean): void {
    if (!dashing) {
      this.dashTrailTimer = 0;
      return;
    }
    ...
    this.particles.spawn(
      ...
      7,
      0.18,
      -this.player.dashX * 42,
      -this.player.dashY * 42,
    );
  }

  private updateFootstepFeedback(): void {
    if (this.player.footstepSerial === this.observedFootstepSerial) return;
    this.observedFootstepSerial = this.player.footstepSerial;
    if (this.saveSystem.data.settings.reducedParticles && this.player.footstepSerial % 2 === 0) return;
    ...
    this.particles.spawn(
      'smoke',
      this.player.x - this.player.lastMove.x * 5 + sideX,
      this.player.y - this.player.lastMove.y * 5 + sideY + this.player.radius * 0.55,
      '#7ea7a3',
      4.5,
      0.16,
      -this.player.motionVx * 0.045,
      -this.player.motionVy * 0.045,
    );
  }

  private rebaseWorldIfNeeded(): void {
    if (Math.abs(this.player.x) < 32768 && Math.abs(this.player.y) < 32768) return;
    ...
    this.particles.pool.forEachActive((item) => {
      item.x -= offsetX; item.y -= offsetY; item.x2 -= offsetX; item.y2 -= offsetY;
    });

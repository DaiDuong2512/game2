 ...
 */
export function viewportSpawnOffset(viewport, angleSample, radiusSample, nearIfFarSample = 1) {
    ...
    let normalizedRadius = Math.sqrt(innerSquared + (outerSquared - innerSquared) * clampUnit(radiusSample));
    ...
    if (normalizedRadius > VIEWPORT_SPAWN_NEAR_OUTER_RADIUS
        ...
        normalizedRadius = Math.sqrt(innerSquared + (nearOuterSquared - innerSquared) * (1 - clampUnit(radiusSample)));
    }
    return {
...
export class EnemySpawner {
    pool = new ObjectPool(() => new Enemy(), 260, 1500);
    data;
    rng;

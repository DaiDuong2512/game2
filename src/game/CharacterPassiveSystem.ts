...
import type { GameData } from '../core/Types.js';
import type { Enemy } from './Entities.js';
import { kaelBloodiedRageRatio } from './Player.js';
import type { ProjectileSystem, ProjectileWorld } from './ProjectileSystem.js';
...
export class CharacterPassiveSystem {
  public nyraBurnPoints = 0;
  public torenForgeKills = 0;
  public miraKillStacks = 0;

  private titanRiftCooldown = 0;
  private novaVoidCooldown = NOVA_VOID_INTERVAL;
  private novaVoidTime = 0;
  private novaVoidX = 0;
  private novaVoidY = 0;
  private elaraShieldCooldown = ELARA_SHIELD_INTERVAL;
  private elaraSoldierAttackCooldown = 0;
  private zarekTrailCooldown = 0;
  private nyraBurnTick = 1;
  private nyraInfernoTime = 0;
  private nyraInfernoX = 0;
  private nyraInfernoY = 0;

  public update(dt: number, world: CharacterPassiveWorld, playerTravel = 0): void {
    const step = Math.max(0, dt);
    this.titanRiftCooldown = Math.max(0, this.titanRiftCooldown - step);
    switch (world.player.character.id) {
      case 'nova': this.updateNova(step, world); break;
      case 'elara': this.updateElara(step, world); break;
      case 'zarek': this.updateZarek(step, world, playerTravel); break;
      case 'nyra-sol': this.updateNyra(step, world); break;
    }
  }

  public onDamageDealt(
    world: CharacterPassiveWorld,
    enemy: Enemy,
  ...
  public onEnemyKilled(world: CharacterPassiveWorld): void {
    ...
    if (world.player.character.id === 'toren-vale') {
      ...
      if (this.torenForgeKills % TOREN_FORGE_KILLS === 0) this.grantTorenWeapon(world);
    }
  }

  public statusText(world: CharacterPassiveWorld): string {
  ...
  private updateNova(dt: number, world: CharacterPassiveWorld): void {
  ...
  private updateZarek(dt: number, world: CharacterPassiveWorld, playerTravel: number): void {
    this.zarekTrailCooldown = Math.max(0, this.zarekTrailCooldown - dt);
    if (playerTravel <= 0.35 || this.zarekTrailCooldown > 0) return;
  ...
  private updateNyra(dt: number, world: CharacterPassiveWorld): void {
  ...
  private triggerNyraInferno(world: CharacterPassiveWorld): void {
  ...
  private triggerTitanRift(world: CharacterPassiveWorld, x: number, y: number, damage: number): void {
    ...
    for (const target of [...world.enemySpatial.queryCircle(x, y, TITAN_RIFT_RADIUS)]) {
      if (!target.active || distanceSquared(x, y, target.x, target.y) > (TITAN_RIFT_RADIUS + target.radius) ** 2) continue;
  ...
  private grantTorenWeapon(world: CharacterPassiveWorld): void {
    ...
    if (mastery && world.weapons.masterWeapon(mastery.config.id)) world.toast(`Luyện Lò · Cường hóa ${mastery.config.name}`);
  }

  private weaponDamage(world: CharacterPassiveWorld, weaponId: string, scale: number): number {
    const weapon = world.data.weaponById.get(weaponId);
    if (!weapon) return 10 * scale * world.player.effectiveDamageMultiplier();

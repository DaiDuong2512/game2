...
import type { GameData, MetaUpgradeConfig, PermanentRewardChoice, PermanentStatId, RunStats } from '../core/Types.js';
...
export class MetaProgression {
  ...
  public constructor(data: GameData, saveSystem: SaveSystem) {
    this.data = data;
    this.saveSystem = saveSystem;
  }

  public cost(config: MetaUpgradeConfig): number {
    const level = this.level(config.id);
    const rawCost = config.baseCost * Math.pow(config.costGrowth, level);
    if (!Number.isFinite(rawCost) || rawCost >= Number.MAX_SAFE_INTEGER) return Number.MAX_SAFE_INTEGER;
    return Math.max(1, Math.round(rawCost));
  }

  public level(id: string): number {
  ...
  public purchase(id: string): boolean {
    ...
    if (this.saveSystem.data.goldReserve < cost) return false;
  ...
  public commitRun(stats: RunStats, stageRewardGold: number, stageRewardShards: number): string[] {
  ...
  public prepareVictoryRewards(stats: RunStats): PermanentRewardChoice[] {
    const save = this.saveSystem.data;
    if (stats.result !== 'victory') return [];
    if (save.pendingPermanentChoices.length > 0) return [...save.pendingPermanentChoices];

    const rng = new RNG((stats.seed ^ Math.imul(stats.stageIndex + 1, 0x9e3779b1)) >>> 0);
    const statsPool: PermanentStatId[] = ['attackSpeed', 'moveSpeed', 'armor', 'damage', 'lifeSteal', 'luck'];
    for (let index = 0; index < 10; index += 1) {
      const stat = rng.pick(statsPool) ?? 'damage';
      save.permanentPoints[stat] = (save.permanentPoints[stat] ?? 0) + 1;
    }

    const labels: Record<PermanentStatId, { title: string; description: string }> = {
  ...
  public claimPermanentReward(choiceId: string): boolean {
    const save = this.saveSystem.data;
  ...
  public pendingPermanentRewards(): readonly PermanentRewardChoice[] {

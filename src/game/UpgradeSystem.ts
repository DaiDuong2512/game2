...
export class UpgradeSystem {
  ...
  private readonly evolutions: EvolutionSystem;
  private readonly passives = new Map<string, PassiveRuntime>();
  private readonly banished = new Set<string>();
  private current: UpgradeOption[] = [];
  private starterCurrent: StarterOption[] = [];
  private currentChoiceLevel: number | null = null;
  public rerolls = 2;
  public banishes = 2;

  public constructor(data: GameData, rng: RNG, player: Player, weapons: WeaponSystem) {
    this.data = data;
    this.rng = rng;
    this.player = player;
  ...
  public generateStarterOptions(): StarterOption[] {
    const buffs: StarterBuff[] = [
    ...
    this.starterCurrent = candidates.map((weapon, index) => {
      ...
      const buff = starterBuffs[index] ?? buffs[index % buffs.length] as StarterBuff;
      return {
  ...
  public applyStarter(optionId: string): boolean {
    const option = this.starterCurrent.find((item) => item.id === optionId);
    if (!option || !this.weapons.addAuxiliaryWeapon(option.weaponId)) return false;
  ...
  public generateOptions(choiceLevel = this.player.level): UpgradeOption[] {
    ...
    if (weaponMilestone) {
      for (const evolution of this.evolutions.eligible(this.weapons, passiveLevels)) {
        ...
        if (!weapon || this.isBanished('evolution', evolution.id)) continue;
        pool.push({
    ...
    if (weaponMilestone) {
      for (const weapon of this.data.weapons) {
        if (this.isBanished('weapon', weapon.id)) continue;
    ...
    if (!weaponMilestone) for (const passive of this.data.passives) {
      if (this.isBanished('passive', passive.id)) continue;
    ...
    if (!weaponMilestone) for (const boost of this.data.upgrades.statBoosts) {
      if (this.isBanished('stat', boost.id)) continue;
      ...
      if (boost.kind === 'heal' && this.player.health >= this.player.stats.get('maxHp') * 0.88) continue;
      pool.push({
        id: `stat:${boost.id}`,
        type: 'stat',
        title: boost.name,
    ...
    const selected: UpgradeOption[] = guaranteedEvolutions.slice(0, 1);
    ...
    while (selected.length < optionCount) {
      const candidates = normalPool.filter((option) =>
        !selected.some((existing) => existing.id === option.id || existing.targetId === option.targetId)
  ...
  public reroll(): UpgradeOption[] | null {
    if (this.rerolls <= 0) return null;
    this.rerolls -= 1;
    return this.generateOptions(this.currentChoiceLevel ?? this.player.level);
  }

  public banish(optionId: string): UpgradeOption[] | null {
    if (this.banishes <= 0) return null;
  ...
  public apply(optionId: string): boolean {
    const option = this.current.find((item) => item.id === optionId);
    if (!option) return false;
    const previousMaxHp = this.player.stats.get('maxHp');
    let applied = false;
    switch (option.type) {
      case 'weapon-new': applied = this.weapons.addAuxiliaryWeapon(option.targetId); break;
      case 'weapon-level': applied = this.weapons.levelWeapon(option.targetId); break;
      case 'weapon-mastery': applied = this.weapons.masterWeapon(option.targetId); break;
      case 'passive-new':
      case 'passive-level': {
        ...
        if (runtime.level >= config.maxLevel && !endless) break;
        runtime.level += 1;
      ...
      case 'evolution': {
        ...
        if (evolution) applied = this.weapons.evolve(evolution.weapon, evolution.id);
        break;
      }
      case 'stat': applied = option.statBoost ? this.applyStatBoost(option.statBoost, option.rarity.multiplier) : false; break;
    }
    if (applied && option.type.startsWith('weapon')) {
      const rarityBonus = Math.max(0, option.rarity.multiplier - 1) * 0.025;
      if (rarityBonus > 0) this.weapons.refineWeapon(option.targetId, rarityBonus);
    } else if (applied && option.type !== 'stat' && option.type !== 'evolution') {
      const rarityBonus = Math.max(0, option.rarity.multiplier - 1) * 0.025;
      if (rarityBonus > 0) this.player.stats.apply('damage', rarityBonus, 'multiply');
    }
    this.player.syncMaxHp(previousMaxHp);
    return applied;
  }

  private applyStatBoost(boost: StatBoostConfig, rarityMultiplier: number): boolean {
    if (boost.kind === 'heal') {
      this.player.heal(this.player.stats.get('maxHp') * boost.value * rarityMultiplier);
      return true;
    }
    if (!boost.stat) return false;
  ...
  private weaponLevelDescription(id: string, nextLevel: number): string {
    const config = this.data.weaponById.get(id);
    const level = config?.levels[nextLevel - 1];
    if (!config || !level) return 'Tăng sức mạnh vũ khí.';
  ...
  private rollRarity(): RarityConfig {
    const luck = this.player.stats.get('luck');
    const choices: WeightedChoice<RarityConfig>[] = this.data.upgrades.rarities.map((rarity) => {
      let weight = rarity.weight;
      if (rarity.id === 'common') weight *= Math.max(0.25, 1 - luck * 1.4);
      if (rarity.id === 'rare') weight *= 1 + luck * 0.7;
      if (rarity.id === 'epic') weight *= 1 + luck * 1.7;
      if (rarity.id === 'legendary') weight *= 1 + luck * 3.2 + (this.player.character.passive.kind === 'extra-choice' ? 0.6 : 0);
      return { item: rarity, weight };
    });
    return this.rng.weighted(choices) ?? this.data.upgrades.rarities[0] as RarityConfig;
  }

  private legendaryRarity(): RarityConfig {
    return this.data.upgrades.rarities.find((rarity) => rarity.id === 'legendary') ?? this.data.upgrades.rarities.at(-1) as RarityConfig;
  }

  private weightedOption(options: readonly UpgradeOption[]): UpgradeOption | undefined {
    if (options.length === 0) return undefined;
    const auxiliaryCount = this.weapons.auxiliaryCount();
    const passiveCount = this.passives.size;
    const choices: WeightedChoice<UpgradeOption>[] = options.map((option) => {
      let weight = 1;
      switch (option.type) {
        case 'weapon-level': weight = 3.8; break;
        case 'weapon-mastery': weight = 3.4; break;
        case 'passive-level': weight = 3.15; break;
        case 'weapon-new': weight = auxiliaryCount === 0 ? 1.35 : auxiliaryCount === 1 ? 0.85 : auxiliaryCount === 2 ? 0.48 : 0; break;
        case 'passive-new': weight = passiveCount < 3 ? 1.6 : passiveCount < 5 ? 0.9 : 0.48; break;
        case 'stat': weight = 1.15; break;
        case 'evolution': weight = 8; break;
      }
      if (this.isEvolutionSynergy(option)) weight *= 2.6;
      return { item: option, weight };
    });
    return this.rng.weighted(choices);
  }

  private isEvolutionSynergy(option: UpgradeOption): boolean {
    for (const evolution of this.data.evolutions) {
      if (this.weapons.evolutionOf(evolution.weapon)) continue;
      if (option.targetId === evolution.passive
        && this.weapons.has(evolution.weapon)
        && this.passiveLevel(evolution.passive) === 0) return true;
      if (option.targetId === evolution.weapon && this.passiveLevel(evolution.passive) > 0) return true;
    }
    return false;
  }

  private isBanished(category: string, id: string): boolean {
    return this.banished.has(`${category}:${id}`);
  }

  public passiveEntries(): ReadonlyArray<PassiveRuntime> {
    return [...this.passives.values()];
  }

  public passiveLevel(id: string): number {
    return this.passives.get(id)?.level ?? 0;
  }

  public scheduledChoiceLevel(): number | null {

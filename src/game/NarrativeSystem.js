...
export class NarrativeSystem {
     ...
     */
    startStage(stageId, characterId) {
        ...
        if (!act)
            throw new Error(`Không tìm thấy hồi truyện ${stage.actId}.`);
    ...
    updateProgress(progress) {
        ...
        if (!stage || this.completionEmitted || this.midStageEmitted)
            return [];
        const safeProgress = Math.min(1, Math.max(0, progress));
        if (safeProgress < stage.midStage.triggerProgress)
            return [];
    ...
    triggerFinalEncounter() {
        const stage = this.activeStage;
        if (!stage || this.completionEmitted || this.finaleEmitted)
            return null;
    ...
    isCodexUnlocked(entry, highestCompletedStage, unlockedCharacters) {
        if (entry.unlock.kind === 'khởi-đầu')
            return true;
        if (!entry.unlock.targetId)
            return false;
        if (entry.unlock.kind === 'mở-nhân-vật')
            return unlockedCharacters.has(entry.unlock.targetId);
    ...
    requireStage(stageId) {
        ...
        if (!stage)
            throw new Error(`Không tìm thấy cốt truyện cho bản đồ ${stageId}.`);
        return stage;
    }
    requireCharacter(characterId) {
        const character = this.characterById.get(characterId);
        if (!character)
            throw new Error(`Không tìm thấy cốt truyện cho nhân vật ${characterId}.`);
        return character;
    }
    toCue(cue, kind, stageId) {

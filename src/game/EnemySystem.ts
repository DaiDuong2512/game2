...
import type { ScaleSnapshot, ElementType } from '../core/Types.js';
...
export interface EnemyWorld {
  ...
  killEnemy(enemy: Enemy, sourceWeaponId: string): void;
  damageStatus(enemy: Enemy, damage: number, element: ElementType, sourceWeaponId: string, status?: 'bleed'): boolean;
  damagePlayer(rawDamage: number, sourceX: number, sourceY: number): void;
  screenShake(amount: number): void;
}

const enemyProjectileColors: Partial<Record<ElementType, string>> = {
  physical: '#f0b87a',
  fire: '#ff7144',
  ice: '#72d8ff',
  lightning: '#6fa8ff',
...
export class EnemySystem {
  private readonly statusAudioCooldown = new Map<SoundId, number>();

  public update(dt: number, world: EnemyWorld): void {
    ...
    for (const enemy of world.spawner.pool.allItems()) {
      ...
      if (enemy.spawnPortalTime > 0) {
        enemy.spawnPortalTime = Math.max(0, enemy.spawnPortalTime - dt);
        const emerged = 1 - enemy.spawnPortalTime / Math.max(0.001, enemy.spawnPortalDuration);
        enemy.alpha = Math.max(0.08, Math.min(1, emerged));
        // Trong lúc trồi khỏi cổng, quái chưa thể di chuyển, bắn hoặc chạm người chơi.
        continue;
      }
      if (enemy.alpha < 1 && enemy.config.ai !== 'burrow') enemy.alpha = 1;

      if (this.updateStatuses(enemy, dt, world)) continue;
      ...
      if (enemy.status.stunTime <= 0 && enemy.status.paralysisTime <= 0) this.updateAI(enemy, dt, world, dx, dy, Math.sqrt(distanceSq));
      this.applySeparation(enemy, dt, world);
      world.terrain?.resolveActor(enemy, previousX, previousY);
      this.contactAttack(enemy, world);
    }
  }

  private updateStatuses(enemy: Enemy, dt: number, world: EnemyWorld): boolean {
    ...
    if (status.bleedTime > 0) {
      ...
      while (status.bleedTick <= 0.0000001) {
        ...
        if (world.damageStatus(enemy, enemy.health * status.bleedDps, 'physical', status.bleedSourceWeapon || fatalSource, 'bleed')) return true;
        fatalSource = status.bleedSourceWeapon || fatalSource;
      }
      if (world.rng.chance(dt * 5)) world.particles.spawn('spark', enemy.x, enemy.y, '#c83e4d', 2.4, 0.25, world.rng.float(-14, 14), world.rng.float(-30, -8));
    } else {
      status.bleedDps = 0;
      status.bleedTick = 1;
    ...
    if (status.burnTime > 0) {
      ...
      if (world.rng.chance(dt * 8)) world.particles.spawn('spark', enemy.x, enemy.y, '#ff7a3c', 2.5, 0.24, world.rng.float(-20, 20), world.rng.float(-40, -10));
    } else {
      status.burnDps = 0;
      status.burnPercent = 0;
      if (status.poisonTime <= 0) status.healingReduction = 0;
    }
    if (status.poisonTime > 0) {
      const activeDt = Math.min(dt, status.poisonTime);
      status.poisonTime -= dt;
      if (status.poisonDps > 0) this.playStatusCue(world, 'poison-tick', 0.14, 0.225);
      const applied = Math.min(enemy.health, status.poisonDps * activeDt);
      enemy.health -= applied;
      world.player?.healFromBossBlessing?.(applied);
      if (world.rng.chance(dt * 5)) world.particles.spawn('smoke', enemy.x, enemy.y, '#6cdf72', 4, 0.4, world.rng.float(-12, 12), world.rng.float(-22, -5));
    } else {
      status.poisonDps = 0;
      if (status.burnTime <= 0) status.healingReduction = 0;
    }
    if (status.poisonCloudTime > 0) {
      ...
      while (status.poisonCloudTick <= 0.0000001) {
        ...
        if (world.damageStatus(
          enemy,
          enemy.health * status.poisonCloudPercent + status.poisonCloudDps,
          'poison',
          status.poisonCloudSourceWeapon || fatalSource,
        )) return true;
        fatalSource = status.poisonCloudSourceWeapon || fatalSource;
      }
      if (world.rng.chance(dt * 7)) world.particles.spawn('smoke', enemy.x, enemy.y, '#7be35c', 5, 0.45, world.rng.float(-14, 14), world.rng.float(-28, -6));
    } else {
    ...
    if (status.slowTime > 0) status.slowTime = Math.max(0, status.slowTime - dt);
    if (status.slowTime <= 0) status.slowFactor = 1;
    if (status.stunTime > 0) status.stunTime -= dt;
    if (status.shockTime > 0) status.shockTime -= dt;
    if (enemy.health <= 0) {
  ...
  private playStatusCue(world: EnemyWorld, cue: SoundId, intensity: number, cooldown: number): void {
    if ((this.statusAudioCooldown.get(cue) ?? 0) > 0) return;
    this.statusAudioCooldown.set(cue, cooldown);
    world.audio?.play(cue, intensity);
  }

  private updateAI(enemy: Enemy, dt: number, world: EnemyWorld, dx: number, dy: number, distance: number): void {
    ...
    const terrainSlow = world.terrain?.movementMultiplier(enemy.x, enemy.y) ?? 1;
    const speed = enemy.speed * slow * ultimateSlow * terrainSlow;
    ...
    switch (enemy.config.ai) {
      ...
      case 'summoner':
        ...
        if (enemy.abilityTimer <= 0) {
          ...
          for (let index = 0; index < count; index += 1) world.spawner.spawnChild(index % 2 ? 'razorling' : 'riftling', enemy.x, enemy.y, world.scaling, 0.72);
      ...
      case 'exploder':
        ...
        if (distance < 66 + world.player.radius) {
          if (enemy.status.blindTime <= 0) world.damagePlayer(enemy.damage * 1.45, enemy.x, enemy.y);
  ...
  private move(enemy: Enemy, dx: number, dy: number, speed: number, dt: number): void {
  ...
  private keepRange(enemy: Enemy, direction: { x: number; y: number }, distance: number, speed: number, dt: number, minimum: number, maximum: number): void {
    if (distance > maximum) this.move(enemy, direction.x, direction.y, speed, dt);
    else if (distance < minimum) this.move(enemy, -direction.x, -direction.y, speed * 0.9, dt);
    else {
  ...
  private updateRanged(
    ...
    const element: ElementType = enemy.config.element ?? (enemy.config.ai === 'mage' ? 'arcane' : 'physical');
    const color = enemyProjectileColors[element] ?? '#f0b87a';
    if (sniper) world.particles.line(enemy.x, enemy.y, world.player.x, world.player.y, '#ef716a', 2, 0.32);
  ...
  private radialVolley(enemy: Enemy, world: EnemyWorld, count: number, damage: number, speed: number, color: string): void {
    for (let index = 0; index < count; index += 1) {
  ...
  private applySeparation(enemy: Enemy, dt: number, world: EnemyWorld): void {
  ...
  private contactAttack(enemy: Enemy, world: EnemyWorld): void {
    if (enemy.status.blindTime > 0 || enemy.status.stunTime > 0 || enemy.status.paralysisTime > 0) return;
    const combined = enemy.radius + world.player.radius;
    if (distanceSquared(enemy.x, enemy.y, world.player.x, world.player.y) > combined * combined || enemy.contactTimer > 0) return;
    enemy.contactTimer = enemy.isBoss ? 0.6 : 0.82;
    world.damagePlayer(enemy.damage, enemy.x, enemy.y);
    if (enemy.config.ai === 'leech') this.healEnemy(enemy, enemy.damage * 1.5);
    const away = normalize(enemy.x - world.player.x, enemy.y - world.player.y);
    enemy.knockbackX += away.x * 120;
    enemy.knockbackY += away.y * 120;
  }

  private healEnemy(enemy: Enemy, amount: number): void {

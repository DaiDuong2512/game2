...
function mixedHash(stageIndex, cellX, cellY) {
...
function segmentCircleEntry(startX, startY, endX, endY, circleX, circleY, radius) {
...
export class TerrainSystem {
    ...
    visible = [];
    visibleDecorations = [];
    cacheX = Number.POSITIVE_INFINITY;
    cacheY = Number.POSITIVE_INFINITY;
    cacheWidth = 0;
    cacheHeight = 0;
    constructor(stage) {
        this.stage = stage;
    }
    update(centerX, centerY, viewport) {
        const width = Math.max(320, viewport.width);
        const height = Math.max(320, viewport.height);
        if (Math.abs(centerX - this.cacheX) < 90
    ...
    firstProjectileBlock(startX, startY, endX, endY, projectileRadius) {
        let earliest = Number.POSITIVE_INFINITY;
        let hit = null;
        for (const feature of this.visible) {
            if (feature.kind === 'water')
                continue;
            const time = segmentCircleEntry(startX, startY, endX, endY, feature.x, feature.y, feature.radius + projectileRadius);
            if (time === null || time >= earliest)
                continue;
            earliest = time;
            hit = feature;
        }
        if (!hit || !Number.isFinite(earliest))
            return null;
        return {
    ...
    featureForCell(cellX, cellY) {
        const hash = mixedHash(this.stage.index, cellX, cellY);
        if (unitFromHash(hash) < 0.17)
            return null;
        ...
        const kind = kindRoll < 0.22 ? 'water' : kindRoll < 0.59 ? 'tree' : 'rock';
    ...
    buildDecorations(centerX, centerY, width, height, features) {

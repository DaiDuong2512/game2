...
export interface SkillWorld extends WeaponWorld {
  ...
  audio: AudioManager;
  weapons?: {
    primaryEntry(): { config: WeaponConfig; runtime: WeaponRuntime } | null;
  };
  data?: {
    evolutionById: ReadonlyMap<string, EvolutionConfig>;
  };
  toast(message: string): void;
}

interface PrimarySkillContext {
  element?: ElementType;
...
export class SkillSystem {
  private ultimateTick = 0;
  private ultimatePulseIndex = 0;
  private ragePulseTick = 0;
  ...
  public update(dt: number, world: SkillWorld): void {
    ...
    if (this.ultimateWasActive && !ultimateActiveAtFrameStart) this.playUltimateAudio(world, 'regen', 0.58);
  ...
  private castActive(world: SkillWorld): void {
  ...
  private primarySkillContext(world: SkillWorld): PrimarySkillContext {
    ...
    if (!entry) return { damageMultiplier: 1 };
  ...
  private damageArea(
    ...
    statusChance: number,
    knockback: number,
    primary: PrimarySkillContext,
    afterHit?: (enemy: Enemy) => void,
  ): number {
    let totalDamage = 0;
    ...
    for (const enemy of candidates) {
      ...
      if (enemy.active) afterHit?.(enemy);
    }
    return totalDamage;
  }

  private castGaleVolley(world: SkillWorld, primary: PrimarySkillContext): void {
    const requestedCount = 9
  ...
  private castEchoPack(world: SkillWorld, primary: PrimarySkillContext): void {
  ...
  private castUltimatePulse(world: SkillWorld): void {
    ...
    const elements = ['fire', 'ice', 'lightning', 'poison'] as const;
    ...
    for (const enemy of candidates) {
      ...
      if (kind === 'forgequake' || kind === 'titanfall') enemy.status.stunTime = Math.max(enemy.status.stunTime, enemy.isBoss ? 0.08 : 0.32);
    }
    if (kind === 'echo-legion') this.spawnEchoLegion(world, pulseIndex);
    this.emitUltimatePulseVfx(world, kind, element, pulseIndex, radius);
    if (element === 'lightning') world.audio.play('lightning', 0.38);
    else if (element === 'fire') world.audio.play('fire', 0.36);
    if (kind === 'titanfall' && pulseIndex === 0) world.screenShake(8);
  ...
  private updateRage(dt: number, world: SkillWorld): void {
    if (!(world.player.rageActive > 0)) return;
    this.ragePulseTick -= dt;
    if (this.ragePulseTick > 0) return;
    ...
    if (immunity) world.particles.spawnAtlas?.(4, world.player.x, world.player.y, 74, 0.24, 0.42);
  }

  private spawnEchoLegion(world: SkillWorld, pulseIndex: number): void {
    const count = 8;
  ...
  private emitActiveCast(world: SkillWorld, kind: string): void {
  ...
  private emitRageCast(world: SkillWorld, characterId: string, bonus: 'extra-projectile' | 'status-immunity'): void {
    const x = world.player.x;
    const y = world.player.y;
  ...
  private emitUltimateCast(world: SkillWorld, kind: string): void {
    const x = world.player.x;
    const y = world.player.y;
  ...
  private emitUltimatePulseVfx(
    world: SkillWorld,
    kind: string,
    element: 'fire' | 'ice' | 'lightning' | 'poison' | 'physical' | 'arcane',
    pulseIndex: number,
    radius: number,
  ): void {
    const x = world.player.x;
    ...
    switch (kind) {
      ...
      case 'elemental-tempest': {
        ...
        if (element === 'poison') {
          for (let index = 0; index < 7; index += 1) world.particles.spawn?.('smoke', impactX, impactY, '#73e47b', 7, 0.54, (index - 3) * 11, -24 - index * 2);
        } else world.particles.spawnAtlas?.(row, impactX, impactY, 118, 0.36, 0.9);
        break;
      }
      case 'plague-night':
        ...
        if (pulseIndex % 3 === 0) world.particles.spawnAtlas?.(3, x, y, 142, 0.44, 0.52, true);
        break;
      case 'echo-legion':
  ...
  private ultimateInterval(kind: string): number {
    switch (kind) {
  ...
  private rollSkillCritical(world: SkillWorld): boolean {
    return world.player.skillCritShards > 0 && world.rng.chance(0.1);
  }

  private playClassSkillAudio(world: SkillWorld, kind: string, intensity: number): void {
    const audio = world.audio as AudioManager & { playClassSkill?: (skillKind: string, value?: number) => void };
    if (typeof audio.playClassSkill === 'function') audio.playClassSkill(kind, intensity);
    else audio.play('skill', intensity);
  }

  private playRageAudio(world: SkillWorld, phase: 'start' | 'loop' | 'end', intensity: number): void {
    const audio = world.audio as AudioManager & { playRagePhase?: (value: 'start' | 'loop' | 'end', level?: number) => void };
    if (typeof audio.playRagePhase === 'function') audio.playRagePhase(phase, intensity);
    else if (phase === 'start') audio.play('rage', intensity);
  }

  private playUltimateAudio(world: SkillWorld, phase: 'cast' | 'regen', intensity: number): void {
    const audio = world.audio as AudioManager & { playUltimatePhase?: (value: 'cast' | 'regen', level?: number) => void };
    if (typeof audio.playUltimatePhase === 'function') audio.playUltimatePhase(phase, intensity);
    else if (phase === 'cast') audio.play('ultimate', intensity);
  }
}

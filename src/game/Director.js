...
// nhân đôi từ nhịp 1,25 trước đây. Hệ số kích cỡ và quái nhỏ khi có Trùm vẫn
// được áp dụng ở lớp lựa chọn phía dưới.
export const ENEMY_SPAWN_RATE_MULTIPLIER = 2.5;
export const BOSS_SMALL_SPAWN_RATE_MULTIPLIER = 2;
export const RANGED_ENEMY_SPAWN_MULTIPLIER = 0.5;
export const ENEMY_SIZE_SELECTION_MULTIPLIERS = Object.freeze({
    small: 2,
    medium: 1.25,
    large: 1.15,
});
...
export function directorSpawnDensity(spawnBase, spawnRate) {
...
export function resolveEnemySizeClass(config) {
    if (config.sizeClass)
        return config.sizeClass;
...
export function enemySizeSelectionMultiplier(config) {
 ...
 */
export function directorChoiceWeight(config, wave, waveCount) {
    ...
    switch (config.ai) {
        case 'melee':
        case 'splitter':
            roleWeight = 1.12 - progress * 0.4;
            break;
        case 'fast':
        case 'flying':
            roleWeight = 1.28 - progress * 0.33;
            break;
        case 'tank':
        case 'shield':
            roleWeight = 0.55 + progress * 0.7;
            break;
        case 'healer':
        case 'summoner':
            roleWeight = 0.28 + progress * 0.82;
            break;
        case 'charger':
        case 'exploder':
        case 'assassin':
        case 'burrow':
            roleWeight = 0.48 + progress * 0.72;
            break;
        case 'ranged':
        case 'sniper':
        case 'mage':
            roleWeight = (0.52 + progress * 0.66) * RANGED_ENEMY_SPAWN_MULTIPLIER;
            break;
        case 'leech':
            roleWeight = 0.65 + progress * 0.45;
            break;
        default:
            roleWeight = 0.8 + progress * 0.2;
            break;
    }
...
export class Director {
    ...
    start(stage, qaMode) {
        ...
        this.candidates = stage.allowedEnemies
            ...
            .filter((item) => Boolean(item && item.tier === 'normal'));
    ...
    update(dt, playerX, playerY, scaling, wave, intermission, activeCount, viewport, bossActive = false) {
        if (!this.stage || intermission > 0)
            return;
        ...
        while (this.budget >= 1 && activeCount + spawned < cap && spawned < maxPerFrame) {
            ...
            const enemy = this.spawner.spawnAround(config.id, playerX, playerY, scaling, eliteRoll ? 1.75 : 1, viewport);
            if (!enemy)
                break;
            if (eliteRoll) {
                enemy.isElite = true;

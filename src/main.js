...
function requiredElement(id) {
    ...
    if (!element)
        throw new Error(`Không tìm thấy thành phần bắt buộc #${id}.`);
    return element;
}
function installMobileControls(game) {
    ...
    stick.addEventListener('pointermove', (event) => { if (event.pointerId === pointerId)
        update(event); });
    const release = (event) => {
...
async function bootstrap() {
    ...
    const screenRoot = requiredElement('screen-root');
    const hudRoot = requiredElement('hud-root');
    const toastRoot = requiredElement('toast-root');
